...
class GitChangeLog:
    """
    Helper class to output a list of logs entries on a bridge changelog format
    ...
    def __init__(
        ...
        self._bridge_repo: Repository = None
    ...
    def _fetch_github_pr(self, pr_number: int) -> PullRequest:
        ...
        """
        try:
            github_repo = self._github.get_repo(BRIDGE_REPO)
            self._bridge_repo = github_repo
    ...
    def _has_commit_migrations(self, git_sha: str) -> bool:
        commit = self._bridge_repo.get_commit(sha=git_sha)
        return any(
            "bridge/migrations/versions/" in file.filename for file in commit.files
        )
    ...
    def _get_pull_request_details(self, git_log: GitLog) -> Dict[str, Any]:
        ...
        pr_type = re.match(BRIDGE_PULL_REQUEST_TYPES, title)
        if pr_type:
    ...
    def _is_risk_pull_request(self, labels: List[Any]) -> bool:
        for label in labels:
            risk_label = re.match(BRIDGE_RISKY_LABELS, label.name)
    ...
    def _parse_change_log(
        ...
        formatted_pr = (
            f"- [#{pr_info.get('id')}]"
            f"(https://github.com/{BRIDGE_REPO}/pull/{pr_info.get('id')}) "
        ...
        for i, log in enumerate(self._logs):
            ...
            if self._show_risk:
                if pr_info.get("is_risky"):
                    result += (
                        f"- [#{log.pr_number}]"
                        f"(https://github.com/{BRIDGE_REPO}/pull/{log.pr_number}) "
                        f"{pr_info.get('title')} (@{github_login})  "
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        for log in self._logs:
            yield {
                ...
                "pr_link": f"https://github.com/{BRIDGE_REPO}/pull/"
                f"{log.pr_number}",
                "message": log.message,
                "time": log.time,
                "author": log.author,
