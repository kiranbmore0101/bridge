...
on the Bridge Slack. People crafting releases and those interested in
...
```bash
    ...
    svn checkout https://dist.apache.org/repos/dist/dev/bridge/ ~/svn/bridge_dev

    svn checkout https://dist.apache.org/repos/dist/release/bridge/ ~/svn/bridge
    cd ~/svn/bridge


    # Add your GPG pub key to KEYS file. Replace "Maxime Beauchemin" with your name
    export BRIDGE_PGP_FULLNAME="Maxime Beauchemin"
    (gpg --list-sigs "${BRIDGE_PGP_FULLNAME}" && gpg --armor --export "${BRIDGE_PGP_FULLNAME}" ) >> KEYS


    # Commit the changes
    svn commit -m "Add PGP keys of new Bridge committer"
...
necessary environment variables. Change your current directory to `bridge/RELEASING`
...
```bash
cd RELEASING
source set_release_env.sh <BRIDGE_RC_VERSION> <PGP_KEY_FULLNAME>
```

Usage (BASH):

```bash
. set_release_env.sh <BRIDGE_RC_VERSION> <PGP_KEY_FULLNAME>
```
...
BRIDGE_RC=1
BRIDGE_GITHUB_BRANCH=1.5
BRIDGE_PGP_FULLNAME=villebro@apache.org
BRIDGE_VERSION_RC=1.5.1rc1
BRIDGE_RELEASE=apache-bridge-1.5.1
BRIDGE_RELEASE_RC=apache-bridge-1.5.1rc1
BRIDGE_RELEASE_TARBALL=apache-bridge-1.5.1-source.tar.gz
BRIDGE_RELEASE_RC_TARBALL=apache-bridge-1.5.1rc1-source.tar.gz
BRIDGE_TMP_ASF_SITE_PATH=/tmp/incubator-bridge-site-1.5.1
-------------------------------
```
...
git checkout master
git pull
git checkout -b ${BRIDGE_GITHUB_BRANCH}
git push origin $BRIDGE_GITHUB_BRANCH
```
...
git checkout master
git pull
git checkout ${BRIDGE_GITHUB_BRANCH}
```
...
```bash
python changelog.py --previous_version 1.4 --current_version ${BRIDGE_GITHUB_BRANCH} changelog
```
...
```bash
python changelog.py --previous_version 1.5.0 --current_version ${BRIDGE_GITHUB_BRANCH} changelog
```

### Set version number

Finally, bump the version number on `bridge-frontend/package.json` (replace with whichever version is being released excluding the RC version):
...
git commit ...
# push new tag
git tag ${BRIDGE_VERSION_RC}
git push origin ${BRIDGE_VERSION_RC}
```
...
release on Bridge's repo (MAJOR.MINOR branch), the following script will clone
the tag and create a signed source tarball from it:

```bash
# make_tarball will use the previously set environment variables
# you can override by passing arguments: make_tarball.sh <BRIDGE_VERSION> <BRIDGE_VERSION_RC> "<PGP_KEY_FULLNAME>"
./make_tarball.sh
```

Note that `make_tarball.sh`:

- By default, the script assumes you have already executed an SVN checkout to `$HOME/svn/bridge_dev`.
  This can be overridden by setting `BRIDGE_SVN_DEV_PATH` environment var to a different svn dev directory
...
```bash
cd ~/svn/bridge_dev/
svn add ${BRIDGE_VERSION_RC}
svn commit -m "Release ${BRIDGE_VERSION_RC}"
svn update
```
...
https://lists.apache.org/thread.html/e60f080ebdda26896214f7d3d5be1ccadfab95d48fbe813252762879@<dev.bridge.apache.org>

To easily send a voting request to Bridge community, still on the `bridge/RELEASING` directory:

```bash
# Note: use Bridge's virtualenv
(venv)$ python generate_email.py vote_pmc
```

The script will generate the email text that should be sent to dev@bridge.apache.org using an email client. The release version and release candidate number are fetched from the previously set environment variables.

Once 3+ binding votes (by PMC members) have been cast and at
least 72 hours have past, you can post a [RESULT] thread:
https://lists.apache.org/thread.html/50a6b134d66b86b237d5d7bc89df1b567246d125a71394d78b45f9a8@%3Cdev.bridge.apache.org%3E

To easily send the result email, still on the `bridge/RELEASING` directory:
...
```bash
cp -r ~/svn/bridge_dev/${BRIDGE_VERSION_RC}/ ~/svn/bridge/${BRIDGE_VERSION}/
cd ~/svn/bridge/
# Rename the RC (0.34.1rc1) to the actual version being released (0.34.1)
for f in ${BRIDGE_VERSION}/*; do mv "$f" "${f/${BRIDGE_VERSION_RC}/${BRIDGE_VERSION}}"; done
svn add ${BRIDGE_VERSION}
svn commit -m "Release ${BRIDGE_VERSION}"
svn update
```
...
```bash
# Go to the root directory of the repo, e.g. `~/src/bridge`
cd ~/src/bridge/
# make sure you're on the correct branch (e.g. 0.34)
git branch
# Create the release tag
git tag -f ${BRIDGE_VERSION}
# push the tag to the remote
git push origin ${BRIDGE_VERSION}
```

### Update CHANGELOG and UPDATING on bridge

Now that we have a final Apache source release we need to open a pull request on Bridge
...
tar xfvz ~/svn/bridge/${BRIDGE_VERSION}/${BRIDGE_RELEASE_TARBALL}
```

Create a virtual environment and install the dependencies

```bash
cd ${BRIDGE_RELEASE_RC}
python3 -m venv venv
source venv/bin/activate
...
```bash
twine upload dist/apache-bridge-${BRIDGE_VERSION}.tar.gz
```
...
The script will generate the email text that should be sent to dev@bridge.apache.org using an email client. The release version is fetched from the previously set environment variables.
