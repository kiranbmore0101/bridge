...
describe.skip('AdhocFilters', () => {
  beforeEach(() => {
    cy.login();
    cy.intercept('GET', '/bridge/filter/table/*/name').as('filterValues');
    cy.intercept('POST', '/bridge/explore_json/**').as('postJson');
    cy.intercept('GET', '/bridge/explore_json/**').as('getJson');
