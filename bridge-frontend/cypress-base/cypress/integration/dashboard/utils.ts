...
export function interceptLog() {
  cy.intercept('/bridge/log/?explode=events&dashboard_id=*').as('logs');
}

export function interceptFav() {
  cy.intercept(`/bridge/favstar/Dashboard/*/select/`).as('select');
}

export function interceptUnfav() {
  cy.intercept(`/bridge/favstar/Dashboard/*/unselect/`).as('unselect');
}
