...
describe('logger middleware', () => {
  ...
  let postStub;
  beforeEach(() => {
    postStub = sinon.stub(BridgeClient, 'post');
  });
  afterEach(() => {
    next.resetHistory();
    postStub.restore();
  ...
  it('should POST an event to /bridge/log/ when called', () => {
    const clock = sinon.useFakeTimers();
    logger(mockStore)(next)(action);
    expect(next.callCount).toBe(0);

    clock.tick(2000);
    expect(BridgeClient.post.callCount).toBe(1);
    expect(BridgeClient.post.getCall(0).args[0].endpoint).toMatch(
      '/bridge/log/',
    );
  });
  ...
  it('should include ts, start_offset, event_name, impression_id, source, and source_id in every event', () => {
    ...
    expect(BridgeClient.post.callCount).toBe(1);
    const { events } = BridgeClient.post.getCall(0).args[0].postPayload;
  ...
  it('should debounce a few log requests to one', () => {
    ...
    expect(BridgeClient.post.callCount).toBe(1);
    expect(
      BridgeClient.post.getCall(0).args[0].postPayload.events,
    ).toHaveLength(3);
  });
  ...
  it('should use navigator.sendBeacon if it exists', () => {
    ...
    expect(endpoint).toMatch('/bridge/log/');
  });
  ...
  it('should pass a guest token to sendBeacon if present', () => {
    ...
    BridgeClient.configure({ guestToken: 'token' });
