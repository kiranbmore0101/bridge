...
import { ColorScheme, SequentialScheme, styled, t } from '@bridge-ui/core';
...
const ColorSchemeControl = ({
  ...
  const currentScheme = useMemo(() => {
    ...
    if (result === 'BRIDGE_DEFAULT') {
      const schemesObject = isFunction(schemes) ? schemes() : schemes;
      result = schemesObject?.BRIDGE_DEFAULT?.id;
    }
    return result;
  ...
  const options = useMemo(() => {
    ...
    const filteredColorOptions = controlChoices.filter(o => {
      ...
      const isValidColorOption =
        option !== 'BRIDGE_DEFAULT' && !allColorOptions.includes(option);
