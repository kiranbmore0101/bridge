...
import { t, BridgeClient, styled } from '@bridge-ui/core';
...
function PropertiesModal({
  ...
  const fetchChartOwners = useCallback(
    async function fetchChartOwners() {
      try {
        const response = await BridgeClient.get({
  ...
  const onSubmit = async (values: {
    ...
    try {
      const res = await BridgeClient.put({
        endpoint: `/api/v1/chart/${slice.slice_id}`,
          ...
          <Button
            ...
            tooltip={
              slice.is_managed_externally
                ? t(
                    "This chart is managed externally, and can't be edited in Bridge",
                  )
                : ''
            }
            cta
