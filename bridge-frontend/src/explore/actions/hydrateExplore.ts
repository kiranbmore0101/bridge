 ...
 * under the License.
 */
import { ControlStateMapping } from '@bridge-ui/chart-controls';
...
export const hydrateExplore =
  ...
  (dispatch: Dispatch, getState: () => ExplorePageState) => {
    ...
    const verifyColorScheme = (type: ColorSchemeType) => {
      ...
      const defaultScheme =
        type === 'CATEGORICAL' ? 'bridgeColors' : 'bridge_seq_1';
    ...
    const exploreState = {
      ...
      can_download: findPermission('can_csv', 'Bridge', user?.roles),
