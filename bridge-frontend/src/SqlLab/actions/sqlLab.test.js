...
describe('async actions', () => {
  ...
  const fetchQueryEndpoint = 'glob:*/bridge/results/*';
  fetchMock.get(
    fetchQueryEndpoint,
    JSON.stringify({ data: mockBigNumber, query: { sqlEditorId: 'dfsadfs' } }),
  );

  const runQueryEndpoint = 'glob:*/bridge/sql_json/';
  ...
  describe('runQuery with query params', () => {
    ...
    it('makes the fetch request', async () => {
      const runQueryEndpointWithParams = 'glob:*/bridge/sql_json/?foo=bar';
      fetchMock.post(
  ...
  describe('postStopQuery', () => {
    const stopQueryEndpoint = 'glob:*/bridge/stop_query/*';
