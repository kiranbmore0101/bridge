...
import { t, BridgeClient } from '@bridge-ui/core';
...
export function estimateQueryCost(queryEditor) {
  return (dispatch, getState) => {
    ...
    const endpoint =
      schema === null
        ? `/bridge/estimate_query_cost/${dbId}/`
        : `/bridge/estimate_query_cost/${dbId}/${schema}/`;
    return Promise.all([
...
export function fetchQueryResults(query, displayLimit) {
  return function (dispatch) {
    ...
    return BridgeClient.get({
      endpoint: `/bridge/results/${query.resultsKey}/?rows=${displayLimit}`,
...
export function runQuery(query) {
  return function (dispatch) {
    ...
    return BridgeClient.post({
      endpoint: `/bridge/sql_json/${search}`,
  ...
  return BridgeClient.post({
    ...
    .catch(() =>
      dispatch(
        addWarningToast(
          t(
            'Unable to migrate table schema state to backend. Bridge will retry ' +
  ...
  return BridgeClient.post({
    ...
    .catch(() =>
      dispatch(
        addWarningToast(
          t(
            'Unable to migrate query state to backend. Bridge will retry later. ' +
              'Please contact your administrator if this problem persists.',
          ),
        ),
      ),
    ...
    return BridgeClient.post({
      ...
      .catch(() =>
        dispatch(
          addWarningToast(
            t(
              'Unable to migrate query editor state to backend. Bridge will retry ' +
                'later. Please contact your administrator if this problem persists.',
            ),
          ),
        ),
...
export function popStoredQuery(urlId) {
  return function (dispatch) {
    return BridgeClient.get({ endpoint: `/kv/${urlId}` })
      .then(({ json }) =>
        dispatch(
          addQueryEditor({
...
export function createCtasDatasource(vizOptions) {
  return dispatch => {
    dispatch(createDatasourceStarted());
    return BridgeClient.post({
      endpoint: '/bridge/get_or_create_table/',
      postPayload: { data: vizOptions },
    })
      .then(({ json }) => {
        dispatch(createDatasourceSuccess(json));
