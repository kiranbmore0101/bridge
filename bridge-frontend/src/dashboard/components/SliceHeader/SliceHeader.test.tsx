...
jest.mock('src/dashboard/components/SliceHeaderControls', () => ({
  ...
  default: (props: any) => (
    <div
      ...
      data-bridge-can-explore={props.bridgeCanExplore}
      data-bridge-can-csv={props.bridgeCanCSV}
...
test('Should render - default props', () => {
  ...
  // @ts-ignore
  delete props.bridgeCanExplore;
  // @ts-ignore
  delete props.bridgeCanCSV;
  // @ts-ignore
...
test('Should render click to edit prompt and run onExploreChart on click', async () => {
  ...
  const history = createMemoryHistory({
    initialEntries: ['/bridge/dashboard/1/'],
  });
  render(
...
test('Should not render click to edit prompt and run onExploreChart on click if bridgeCanExplore=false', () => {
  const props = createProps({ bridgeCanExplore: false });
  const history = createMemoryHistory({
    initialEntries: ['/bridge/dashboard/1/'],
  });
  render(
  ...
  userEvent.click(screen.getByText('Vaccine Candidates per Phase'));
  expect(history.location.pathname).toMatch('/bridge/dashboard');
});
