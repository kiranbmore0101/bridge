...
import * as BridgeCore from '@bridge-ui/core';
...
test('should render - FeatureFlag disabled', async () => {
  ...
  expect(spyColorSchemeControlWrapper).toBeCalledWith(
    expect.objectContaining({ colorScheme: 'bridgeColors' }),
    {},
  );
});
...
test('submitting with onlyApply:false', async () => {
  const put = jest.spyOn(BridgeCore.BridgeClient, 'put');
...
test('should show all roles', async () => {
  ...
  const getSelect = () =>
    screen.getByRole('combobox', { name: BridgeCore.t('Roles') });
...
test('should show active owners with dashboard rbac', async () => {
  ...
  const open = () => waitFor(() => userEvent.click(getSelect()));
  const getSelect = () =>
    screen.getByRole('combobox', { name: BridgeCore.t('Owners') });
  ...
  expect(options[0]).toHaveTextContent('Bridge Admin');
});
