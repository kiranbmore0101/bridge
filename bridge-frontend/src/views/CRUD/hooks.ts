...
import { makeApi, BridgeClient, t, JsonObject } from '@bridge-ui/core';
...
import BridgeText from 'src/utils/textUtils';
...
export const copyQueryLink = (
  ...
  copyTextToClipboard(() =>
    Promise.resolve(
      `${window.location.origin}/bridge/sqllab?savedQueryId=${id}`,
    ),
  )
    .then(() => {
...
export const getDatabaseImages = () => BridgeText.DB_IMAGES;

export const getConnectionAlert = () => BridgeText.DB_CONNECTION_ALERTS;
export const getDatabaseDocumentationLinks = () =>
  BridgeText.DB_CONNECTION_DOC_LINKS;
