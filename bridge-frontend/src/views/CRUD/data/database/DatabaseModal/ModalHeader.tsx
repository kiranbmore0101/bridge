...
const bridgeTextDocs = getDatabaseDocumentationLinks();

export const DOCUMENTATION_LINK = bridgeTextDocs
  ? bridgeTextDocs.support
  : 'https://bridge.apache.org/docs/databases/installing-database-drivers';

const irregularDocumentationLinks = {
  postgresql: 'https://bridge.apache.org/docs/databases/postgres',
  mssql: 'https://bridge.apache.org/docs/databases/sql-server',
  gsheets: 'https://bridge.apache.org/docs/databases/google-sheets',
};
...
const documentationLink = (engine: string | undefined) => {
  ...
  if (bridgeTextDocs) {
    // override doc link for bridge_txt yml
    return bridgeTextDocs[engine] || bridgeTextDocs.default;
  }

  if (!irregularDocumentationLinks[engine]) {
    return `https://bridge.apache.org/docs/databases/${engine}`;
  }
  ...
  const useSqlAlchemyFormHeader = (
    <StyledFormHeader>
      ...
      <p className="helper-bottom">
        ...
        <a
          href={bridgeTextDocs?.default || DOCUMENTATION_LINK}
          target="_blank"
