...
import { t, BridgeClient, isDefined } from '@bridge-ui/core';
...
const v1ChartDataRequest = async (
  ...
  return BridgeClient.post(querySettings);
};
...
export function redirectSQLLab(formData) {
  return dispatch => {
    getChartDataRequest({ formData, resultFormat: 'json', resultType: 'query' })
      .then(({ json }) => {
        const redirectUrl = '/bridge/sqllab/';
        const payload = {
          datasourceKey: formData.datasource,
          sql: json.result[0].query,
        };
        BridgeClient.postForm(redirectUrl, {
...
export function refreshChart(chartKey, force, dashboardId) {
  return (dispatch, getState) => {
    ...
    const timeout =
      getState().dashboardInfo.common.conf.BRIDGE_WEBSERVER_TIMEOUT;
...
export const getDatasourceSamples = async (
  ...
  try {
    ...
    const response = await BridgeClient.post({
