...
test('navigates to external URL', () => {
  render(
    <GenericLink to="https://bridge.apache.org/">
      Link to external website
    </GenericLink>,
    { useRouter: true },
  );
  const externalLink = screen.getByTestId('external-link');
  expect(externalLink).toHaveAttribute('href', 'https://bridge.apache.org/');
});

test('navigates to external URL without host', () => {
  render(
    <GenericLink to="bridge.apache.org/">
      Link to external website
    </GenericLink>,
    { useRouter: true },
  );
  const externalLink = screen.getByTestId('external-link');
  expect(externalLink).toHaveAttribute('href', '//bridge.apache.org/');
});
