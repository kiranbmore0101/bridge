...
Common table applications in Bridge:
...
you can register `onDragOver` and `onDragDrop` event handlers on the destination component. In the `onDragDrop` handler you can check for `BRIDGE_TABLE_COLUMN`
...
const handleDrop = (ev:Event) => {
  const json = ev.dataTransfer?.getData?.(BRIDGE_TABLE_COLUMN);
