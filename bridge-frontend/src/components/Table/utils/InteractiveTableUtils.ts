...
import { BRIDGE_TABLE_COLUMN } from 'src/components/Table';
...
export default class InteractiveTableUtils {
  ...
  handleColumnDragStart = (ev: DragEvent): void => {
    ...
    ev?.dataTransfer?.setData(BRIDGE_TABLE_COLUMN, JSON.stringify(dragData));
  };

  handleDragDrop = (ev: DragEvent): void => {
    const data = ev.dataTransfer?.getData?.(BRIDGE_TABLE_COLUMN);
    if (data) {
      ev.preventDefault();
