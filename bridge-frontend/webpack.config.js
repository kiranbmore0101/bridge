...
// output dir
const BUILD_DIR = path.resolve(__dirname, '../bridge/static/assets');
...
if (isDevMode) {
  // A Bridge webpage normally includes two JS bundles in dev, `theme.ts` and
...
const config = {
  ...
  resolve: {
    // resolve modules from `/bridge_frontend/node_modules` and `/bridge_frontend`
  ...
  module: {
    rules: [
      ...
      },
      {
        ...
        exclude: [/bridge-ui.*\/node_modules\//, /\.test.jsx?$/],
        include: [
...
Object.entries(packageConfig.dependencies).forEach(([pkg, relativeDir]) => {
  ...
  if (/^@bridge-ui/.test(pkg) && fs.existsSync(srcPath)) {
    console.log(`[Bridge Plugin] Use symlink source for ${pkg} @ ${dir}`);
...
if (isDevMode) {
  ...
  config.devServer = {
    ...
    // and proxy everything else to Bridge backend
    proxy: [
