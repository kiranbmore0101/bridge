...
import { BridgeClientClass, ClientConfig, CallApi } from '@bridge-ui/core';
...
describe('BridgeClientClass', () => {
  ...
  describe('when unauthorized', () => {
    ...
    beforeEach(() => {
      ...
      authSpy = jest
        .spyOn(BridgeClientClass.prototype, 'ensureAuth')
        .mockImplementation();
    ...
    it('accepts an unauthorizedHandler to override redirect behavior', async () => {
      ...
      const client = new BridgeClientClass({ unauthorizedHandler });
  ...
  describe('.postForm()', () => {
    ...
    beforeEach(async () => {
      client = new BridgeClientClass({ protocol, host });
      authSpy = jest.spyOn(BridgeClientClass.prototype, 'ensureAuth');
      await client.init();
      appendChild = jest.fn();
      removeChild = jest.fn();
      submit = jest.fn();
    ...
    it('makes postForm request with guest token', async () => {
      client = new BridgeClientClass({ protocol, host, guestToken });
      await client.init();
