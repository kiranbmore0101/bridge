...
export default async function callApi({
  ...
  if (
    ...
    let bridgeCache: Cache | null = null;
    try {
      bridgeCache = await caches.open(CACHE_KEY);
      ...
      // If bridge is in an iframe and third-party cookies are disabled, caches.open throws
    }

    const response = await fetchWithRetry(url, request);

    if (bridgeCache && response.status === HTTP_STATUS_NOT_MODIFIED) {
