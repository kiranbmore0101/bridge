...
const { bridgePort = 8088, bridge: bridgeUrl = null } = parsedArgs;
const backend = (bridgeUrl || `http://localhost:${bridgePort}`).replace(
  '//+$/',
  '',
...
function toDevHTML(originalHtml) {
  ...
  if (manifest) {
    ...
    // in bridge/templates/bridge/partials/asset_bundle.html
    html = html.replace(
