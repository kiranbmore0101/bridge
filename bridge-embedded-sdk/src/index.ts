...
import { Switchboard } from '@bridge-ui/switchboard';
...
export type EmbedDashboardParams = {
  /** The id provided by the embed configuration UI in Bridge */
  id: string
  /** The domain where Bridge can be located, with protocol, such as: https://bridge.example.com */
  bridgeDomain: string
...
/**
 * Embeds a Bridge dashboard into the page using an iframe.
 */
export async function embedDashboard({
  ...
  function log(...info: unknown[]) {
    if (debug) {
      console.debug(`[bridge-embedded-sdk][dashboard ${id}]`, ...info);
    }
  }
  ...
  async function mountIframe(): Promise<Switchboard> {
    return new Promise(resolve => {
      ...
      iframe.addEventListener('load', () => {
        ...
        resolve(new Switchboard({ port: ourPort, name: 'bridge-embedded-sdk', debug }));
      });

      iframe.src = `${bridgeDomain}/embedded/${id}${dashboardConfig}${filterConfigUrlParams}`;
