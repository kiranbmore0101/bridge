...
- [github.com/apache-bridge](https://github.com/apache-bridge) is the
  ...
  small tools, forks and Bridge-related experimental ideas.
...
Bridge could always use better documentation,
whether as part of the official Bridge docs,
...
text strings from Bridge's UI. You can jump in to the existing
language dictionaries at
`bridge/translations/<language_code>/LC_MESSAGES/messages.po`, or
even create a dictionary for a new language altogether.
See [Translating](#translating) for more details.

### Ask Questions

There is a dedicated [`apache-bridge` tag](https://stackoverflow.com/questions/tagged/apache-bridge) on [StackOverflow](https://stackoverflow.com/). Please use it when asking questions.
...
**Feature/Large changes:** If you intend to change the public API, or make any non-trivial changes to the implementation, we require you to file a new issue as `#SIP` (Bridge Improvement Proposal). This lets us reach an agreement on your proposal before you put significant effort into it. You are welcome to submit a PR along with the SIP (sometimes necessary for demonstration), but we will not review/merge the code until the SIP is approved.
...
| `#SIP`          | Bridge Improvement Proposal                                                                                                           | N/A                                                                                                                                               |
...
When creating a PR, if you're aiming to have it included in a specific release, please tag it with the version label. For example, to have a PR considered for inclusion in Bridge 1.1 use the label `v1.1`.

## Reporting a Security Vulnerability

Please report security vulnerabilities to private@bridge.apache.org.

In the event a community member discovers a security flaw in Bridge, it is important to follow the [Apache Security Guidelines](https://www.apache.org/security/committers.html) and release a fix as quickly as possible before public disclosure. Reporting security vulnerabilities through the usual GitHub Issues channel is not ideal as it will publicize the flaw before a fix can be applied.
...
The latest documentation and tutorial are available at https://bridge.apache.org/.
...
Make sure your machine meets the [OS dependencies](https://bridge.apache.org/docs/installation/installing-bridge-from-scratch#os-dependencies) before following these steps.
...
FLASK_ENV=development bridge run -p 8088 --with-threads --reload --debugger
```
...
# Install bridge pip packages and setup env only
$ make bridge
...
via `.flaskenv`, however if needed, it should be set to `bridge.app:create_app()`**

If you have made changes to the FAB-managed templates, which are not built the same way as the newer, React-powered front-end assets, you need to start the app without the `--with-threads` argument like so:
`FLASK_ENV=development bridge run -p 8088 --reload --debugger`
...
This feature is only available on Python 3. When debugging your application, you can have the server logs sent directly to the browser console using the [ConsoleLog](https://github.com/betodealmeida/consolelog) package. You need to mutate the app, by adding the following to your `config.py` or `bridge_config.py`:
...
```bash
FLASK_ENV=development gunicorn "bridge.app:create_app()" -k "geventwebsocket.gunicorn.workers.GeventWebSocketWorker" -b 127.0.0.1:8088 --reload
```
...
For those interested, you may also try out [avn](https://github.com/nvm-sh/nvm#deeper-shell-integration) to automatically switch to the node version that is required to run Bridge frontend.
...
1. [run Bridge locally](#flask-server) using Flask, on port `8088` — but don't access it directly,<br/>
   ```bash
   # Install Bridge and dependencies, plus load your virtual environment first, as detailed above.
   FLASK_ENV=development bridge run -p 8088 --with-threads --reload --debugger
   ```
...
npm run dev-server -- --bridgePort=8081

# Proxy to a remote backend but serve local assets
npm run dev-server -- --bridge=https://bridge-dev.example.com
```

The `--bridge=` option is useful in case you want to debug a production issue or have to setup Bridge behind a firewall. It allows you to run Flask server in another environment while keep assets building locally for the best developer experience.
...
`bridge-frontend/package-lock.json` is updated according to `npm`-prescribed
best practices.

#### Feature flags

Bridge supports a server-wide feature flag system, which eases the incremental development of features. To add a new feature flag, simply modify `bridge_config.py` with something like the following:
...
If you want to use the same flag in the client code, also add it to the FeatureFlag TypeScript enum in [@bridge-ui/core](https://github.com/apache-bridge/bridge-ui/blob/master/packages/bridge-ui-core/src/utils/featureFlags.ts). For example,
...
`bridge/config.py` contains `DEFAULT_FEATURE_FLAGS` which will be overwritten by
those specified under FEATURE_FLAGS in `bridge_config.py`. For example, `DEFAULT_FEATURE_FLAGS = { 'FOO': True, 'BAR': False }` in `bridge/config.py` and `FEATURE_FLAGS = { 'BAR': True, 'BAZ': True }` in `bridge_config.py` will result
...
Bridge uses Git pre-commit hooks courtesy of [pre-commit](https://pre-commit.com/). To install run the following:
...
```bash
export BRIDGE_CONFIG=tests.integration_tests.bridge_test_config
export BRIDGE_TESTENV=true
export CYPRESS_BASE_URL="http://localhost:8081"
bridge db upgrade
bridge load_test_users
bridge load-examples --load-test-data
bridge init
...
Enter the bridge_app container
...
```
{
    ...
    "configurations": [
        {
            "name": "Attach to Bridge App in Docker Container",
            "type": "python",
            "request": "attach",
            "connect": {
                "host": "127.0.0.1",
...
```
kubectl port-forward  pod/bridge-<some random id> 5678:5678
```
...
Bridge includes a [Storybook](https://storybook.js.org/) to preview the layout/styling of various Bridge components, and variations thereof. To open and view the Storybook:
...
When contributing new React components to Bridge, please try to add a Story alongside the component's `jsx/tsx` file.

## Translating

We use [Flask-Babel](https://python-babel.github.io/flask-babel/) to translate Bridge.
...
```javascript
import { t, tn } from "@bridge-ui/translation";
```

### Enabling language selection

Add the `LANGUAGES` variable to your `bridge_config.py`. Having more than one
...
1. update the template file `bridge/translations/messages.pot` with current application strings.
2. update language files with the new extracted strings.

You can then translate the strings gathered in files located under
`bridge/translation`, where there's one per language. You can use [Poedit](https://poedit.net/features)
...
```bash
pybabel compile -d bridge/translations
```
...
```bash
pip install -r bridge/translations/requirements.txt
pybabel init -i bridge/translations/messages.pot -d bridge/translations -l LANGUAGE_CODE
```
...
1. Create Models and Views for the datasource, add them under bridge folder, like a new my_models.py
...
1. Specify this variable to add the datasource model and from which module it is from in config.py:
   ...
   ```python
   ADDITIONAL_MODULE_DS_MAP = {'bridge.my_models': ['MyDatasource', 'MyOtherDatasource']}
   ```

   This means it'll register MyDatasource and MyOtherDatasource in bridge.my_models module in the source registry.

### Visualization Plugins

The topic of authoring new plugins, whether you'd like to contribute
it back or not has been well documented in the
[the documentation](https://bridge.apache.org/docs/contributing/creating-viz-plugins), and in [this blog post](https://preset.io/blog/building-custom-viz-plugins-in-bridge-v2).

To contribute a plugin to Bridge, your plugin must meet the following criteria:
...
- The plugin should use appropriate namespacing, e.g. a folder name of `plugin-chart-whatever` and a package name of `@bridge-ui/plugin-chart-whatever`
...
1. Generate the migration file
   ...
   ```bash
   bridge db migrate -m 'add_metadata_column_to_annotation_model'
   ```
...
2. Pick one of them as the parent revision, open the script for the other revision
   ...
   Alternatively you may also run `bridge db merge` to create a migration script
   just for merging the heads.

   ```bash
   bridge db merge {HASH1} {HASH2}
   ```
...
- Start up a celery worker
  ...
  ```shell script
  celery --app=bridge.tasks.celery_app:app worker -Ofair
  ```
