...
The bridge cli allows you to import and export datasources from and to YAML. Datasources include
...
```
bridge export_datasources
```

To save your datasources to a file run:

```
bridge export_datasources -f <filename>
```
...
```
bridge export_datasource_schema
```
...
```
bridge import_datasources -p <path or filename>
```
...
```
bridge import_datasources -p <path> -r
```
...
```
bridge import_datasources -p <path / filename> -s columns,metrics
```

This will sync all metrics and columns for all datasources found in the `<path /filename>` in the
Bridge meta database. This means columns and metrics not specified in YAML will be deleted. If you
