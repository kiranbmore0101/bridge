...
Bridge and provides additional context.
...
issue within Bridge, but instead a problem with the underlying
...
```
Bridge encountered an error while running a command.
```

Something unexpected happened, and Bridge encountered an error while
running a command. Please reach out to your administrator.

## Issue 1011

```
Bridge encountered an unexpected error.
```

Something unexpected happened in the Bridge backend. Please reach out
...
Your instance of Bridge doesn't have a results backend configured, which is needed for asynchronous queries. Please contact an administrator for further assistance.
...
The submitted query might be too complex to run under the time limit defined by your Bridge administrator. Please double check your query and verify if it can be optimized. Alternatively, contact your administrator to increase the timeout period.
