---
title: Exploring Data in Bridge
hide_title: true
sidebar_position: 2
version: 1
---
...
## Exploring Data in Bridge

In this tutorial, we will introduce key concepts in Apache Bridge through the exploration of a
...
[GitHub](https://raw.githubusercontent.com/apache-bridge/examples-data/master/tutorial_flights.csv).
In the Bridge menu, select **Data ‣ Upload a CSV**.
...
By default, Apache Bridge only shows the last week of data. In our example, we want to visualize all
...
SUM aggregate. Note that Apache Bridge will indicate the type of the metric by the symbol on the
...
- Set the Annotation Source as Bridge annotation
...
In this section, we are going to explore the Advanced Analytics feature of Apache Bridge that
