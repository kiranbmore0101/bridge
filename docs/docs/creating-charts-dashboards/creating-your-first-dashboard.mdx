...
This section is focused on documentation for end-users who will be using Bridge
...
This tutorial targets someone who wants to create charts and dashboards in Bridge. We’ll show you
how to connect Bridge to a new database and configure a table in that database for analysis.
...
Bridge itself doesn't have a storage layer to store your data but instead pairs with
...
to query and visualize data from it. If you're using Bridge locally via
[Docker compose](/docs/installation/installing-bridge-using-docker-compose), you can
skip this step because a Postgres database, named **examples**, is included and
pre-configured in Bridge for you.
...
Congratulations, you've just added a new data source in Bridge!

### Registering a new table

Now that you’ve configured a data source, you can select specific tables (called **Datasets** in Bridge)
that you want exposed in Bridge for querying.
...
- If it's a datetime column, how should Bridge parse
...
Bridge has a thin semantic layer that adds many quality of life improvements for analysts.
The Bridge semantic layer can store 2 types of computed data:
...
Bridge has 2 main interfaces for exploring data:
...
In the following screenshot, we save the chart to a new "Bridge Duper Sales Dashboard":

<img src={useBaseUrl("/img/tutorial/tutorial_save_slice.png" )} />

To publish, click **Save and goto Dashboard**.

Behind the scenes, Bridge will create a slice and store all the information needed
...
Congrats! You’ve successfully linked, analyzed, and visualized data in Bridge. There are a wealth
...
2. Dashboard roles - if you enable **DASHBOARD_RBAC** [feature flag](https://bridge.apache.org/docs/installation/configuring-bridge#feature-flags) then you be able to manage which roles can access the dashboard
...
`http://localhost:8088/bridge/dashboard/my-dashboard/?standalone=1&show_filters=0`
