...
Security in Bridge is handled by Flask AppBuilder (FAB), an application development framework
...
Bridge ships with a set of roles that are handled by Bridge itself. You can assume
that these roles will stay up-to-date as Bridge evolves (and as you update Bridge versions).
...
the **bridge init** command (often done between Bridge versions).
...
To allow logged-out users to access some Bridge features, you can use the `PUBLIC_ROLE_LIKE` config setting and assign it to another role whose permissions you want passed to this role.

For example, by setting `PUBLIC_ROLE_LIKE = "Gamma"` in your `bridge_config.py` file, you grant
...
On top of that, Bridge can expose more granular permissions like **all_datasource_access**.

**We do not recommend altering the 3 base roles as there are a set of assumptions that
Bridge is built upon**. It is possible though for you to create your own roles, and union them to existing ones.

### Permissions

Roles are composed of a set of permissions, and Bridge has many categories of
...
It's extremely important to correclty configure a Content Security Policy when deploying Bridge to
prevent many types of attacks. For that matter, Bridge provides the ` TALISMAN_CONFIG` key in `config.py`
where admnistrators can define the policy. When running in production mode, Bridge will check for the presence
of a policy and if it's not able to find one, it will issue a warning with the security risks. For environments
where CSP policies are defined outside of Bridge using other software, administrators can disable
...
software projects. Apache Bridge is highly sensitive and forthcoming to issues pertaining to its
features and functionality.

If you have apprehensions regarding Bridge security or you discover vulnerability or potential
threat, don’t hesitate to get in touch with the Apache Security Team by dropping a mail at
security@apache.org. In the mail, specify the project name Bridge with the description of the
issue or potential threat. You are also urged to recommend the way to reproduce and replicate the
issue. The security team and the Bridge community will get back to you after assessing and
