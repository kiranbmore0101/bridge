...
A native connector to Druid ships with Bridge (behind the `DRUID_IS_ACTIVE` flag) but this is
...
Common aggregations or Druid metrics can be defined and used in Bridge. The first and simpler use
...
Druid supports post aggregation and this works in Bridge. All you have to do is create a metric,
