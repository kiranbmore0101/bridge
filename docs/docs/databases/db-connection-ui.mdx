...
1. `parameters_schema`: a Marshmallow schema defining the parameters needed to configure the database. For Postgres this includes username, password, host, port, etc. ([see](https://github.com/apache/bridge/blob/accee507c0819cd0d7bcfb5a3e1199bc81eeebf2/bridge/db_engine_specs/base.py#L1309-L1320)).
...
- `validate_parameters(cls, parameters)`: this method is used for `onBlur` validation of the form. It should return a list of `BridgeError` indicating which parameters are missing, and which parameters are definitely incorrect ([example](https://github.com/apache/bridge/blob/accee507c0819cd0d7bcfb5a3e1199bc81eeebf2/bridge/db_engine_specs/base.py#L1404)).
