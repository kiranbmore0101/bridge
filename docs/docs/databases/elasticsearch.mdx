...
set Bridge’s row limit on config
...
By default, Bridge uses UTC time zone for elasticsearch query. If you need to specify a time zone,
...
the bridge will use the `DATETIME_PARSE` function for conversion.
