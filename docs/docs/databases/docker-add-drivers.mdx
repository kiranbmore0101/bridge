...
Bridge requires a Python database driver to be installed for each additional type of database you
want to connect to. When setting up Bridge locally via `docker-compose`, the drivers and packages
contained in
[requirements.txt](https://github.com/kiranbmore0101/bridge/blob/master/requirements.txt) and
[requirements-dev.txt](https://github.com/kiranbmore0101/bridge/blob/master/requirements-dev.txt)
...
In the example, we'll walk through the process of installing a MySQL driver in Bridge.
...
Docker containers for bridge.

In order to address this, the Bridge `docker compose` setup comes with a mechanism for you to
...
The other option is to start Bridge via Docker Compose is using the recipe in `docker-compose-non-dev.yml`, which will use pre-built frontend assets and skip the building of front-end assets:
...
We can now create a Datasource in Bridge that can be used to connect to a MySQL instance. Assuming
...
You can repeat this process for every database you want bridge to be able to connect to.
