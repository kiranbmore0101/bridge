...
install new database drivers when setting up Bridge locally via docker-compose.
...
When adding a new BigQuery connection in Bridge, you'll need to add the GCP Service Account
...
2. In Bridge, you can either upload that JSON or add the JSON blob in the following format (this should be the content of your credential JSON file):
...
To be able to upload CSV or Excel files to BigQuery in Bridge, you'll need to also add the
