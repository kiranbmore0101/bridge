...
Not in the Explore or Visualization UI. A Bridge SQLAlchemy datasource can only be a single table
or a view.
...
expressions. The limitation there is your database performance as Bridge effectively will run a
query on top of your query (view). A good practice may be to limit yourself to joining your main
large table to one or many small tables only, and avoid using _GROUP BY_ where possible as Bridge
...
serve it in an interactive fashion to provide a good user experience in Bridge.
...
It can be gigantic! Bridge acts as a thin layer above your underlying databases or data engines.
...
For running long query from Sql Lab, by default Bridge allows it run as long as 6 hours before it
...
from Bridge server (which is processing long queries), these web servers will send 504 status code
to clients directly. Bridge has a client-side timeout limit to address this issue. If query didn’t
come back within client-side timeout (60 seconds by default), Bridge will display warning message
to avoid gateway timeout message. If you have a longer gateway timeout limit, you can change the
timeout settings in **bridge_config.py**:

```
BRIDGE_WEBSERVER_TIMEOUT = 60
```
...
to **bridge_config.py** at the key MAPBOX_API_KEY:
...
**Why does ‘flask fab’ or bridge freezed/hung/not responding when started (my home directory is
NFS mounted)?**

By default, Bridge creates and uses an SQLite database at `~/.bridge/bridge.db`. SQLite is
...
You can override this path using the **BRIDGE_HOME** environment variable.

Another workaround is to change where bridge stores the sqlite database by adding the following in
`bridge_config.py`:

```
SQLALCHEMY_DATABASE_URI = 'sqlite:////new/location/bridge.db'
```

You can read more about customizing Bridge using the configuration file
[here](/docs/installation/configuring-bridge).

### What if the table schema changed?

Table schemas evolve, and Bridge needs to reflect that. It’s pretty common in the life cycle of a
dashboard to want to add a new dimension or metric. To get Bridge to discover your new columns,
...
### What database engine can I use as a backend for Bridge?

To clarify, the database backend is an OLTP database used by Bridge to store its internal
information like your list of users, slices and dashboard definitions.

Bridge is tested using Mysql, Postgresql and Sqlite for its backend. It’s recommended you install
Bridge on one of these database server for production.
...
fine as a datasource for Bridge, just not as the OLTP backend.
...
### Does Bridge work with [insert database engine here]?
...
For a database engine to be supported in Bridge through the SQLAlchemy connector, it requires
...
database to be supported is through the Bridge Database Engine Specification interface. This
...
- date-related SQL function that allow Bridge to fetch different time granularities when running
  time-series queries
- whether the engine supports subqueries. If false, Bridge may run 2-phase queries to compensate
  for the limitation
...
Beyond the SQLAlchemy connector, it’s also possible, though much more involved, to extend Bridge
...
### Does Bridge offer a public API?
...
[SIP-17](https://github.com/kiranbmore0101/bridge/issues/7259) and constant progress has been
...
made available under **/swagger/v1** by enabling the following flag in `bridge_config.py`:

```
FAB_API_SWAGGER_UI = True
```

There are other undocumented [private] ways to interact with Bridge programmatically that offer no
guarantees and are not recommended but may fit your use case temporarily:

- using the ORM (SQLAlchemy) directly
- using the internal FAB ModelView API (to be deprecated in Bridge)
