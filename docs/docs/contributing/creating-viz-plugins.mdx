...
Visualizations in Bridge are implemented in JavaScript or TypeScript. Bridge
comes preinstalled with several visualizations types (hereafter "viz plugins") that
can be found under the `bridge-frontend/plugins` directory. Viz plugins are added to
the application in the `bridge-frontend/src/visualizations/presets/MainPreset.js`.
The Bridge project is always happy to review proposals for new high quality viz
plugins. However, for highly custom viz types it is recommended to maintain a fork
of Bridge, and add the custom built viz plugins by hand.
...
To get started, you need the Bridge Yeoman Generator. It is recommended to use the
version of the template that ships with the version of Bridge you are using. This
can be installed by doing the following:

```bash
npm i -g yo
cd bridge-frontend/packages/generator-bridge
npm i
npm link
```

After this you can proceed to create your viz plugin. Create a new directory for your
viz plugin with the prefix `bridge-plugin-chart` and run the Yeoman generator:
...
$ yo @bridge-ui/bridge
   ...
   __'.___.'__
 ´   `  |° ´ Y `
? Package name: bridge-plugin-chart-hello-world
...
? What type of chart would you like? Time-series chart
   ...
   create src/types.ts
   create src/BridgePluginChartHelloWorld.tsx
...
To add the package to Bridge, go to the `bridge-frontend` subdirectory in your
Bridge source folder run

```bash
npm i -S /tmp/bridge-plugin-chart-hello-world
```

If you publish your package to npm, you can naturally install directly from there, too.
After this edit the `bridge-frontend/src/visualizations/presets/MainPreset.js`
and make the following changes:

```js
import { BridgePluginChartHelloWorld } from 'bridge-plugin-chart-hello-world';
```
...
```js
new BridgePluginChartHelloWorld().configure({ key: 'ext-hello-world' }),
```

After that the viz plugin should show up when you run Bridge, e.g. the development
server:
