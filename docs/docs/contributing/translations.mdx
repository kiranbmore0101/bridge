...
for your target language already exists in the `bridge/translations` directory:

```bash
ls bridge/translations | grep -E "^[a-z]{2}\/"
```
...
```bash
pybabel init -i bridge/translations/messages.pot -d bridge/translations -l fi
```
...
codebase via PRs. To update the template file `bridge/translations/messages.pot`
with current application strings, run the following command:

```bash
pybabel extract -F bridge/translations/babel.cfg -o bridge/translations/messages.pot -k _ -k __ -k t -k tn -k tct .
```
...
```bash
 pybabel update -i bridge/translations/messages.pot -d bridge/translations --ignore-obsolete
```

You can then translate the strings gathered in files located under
`bridge/translation`, where there's one folder per language. You can use [Poedit](https://poedit.net/features)
...
case of the Finnish translation, this would be `bridge/translations/fi/LC_MESSAGES/messages.po`.
