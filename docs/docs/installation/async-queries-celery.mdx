...
seconds), it is necessary to configure an asynchronous backend for Bridge which consists of:

- one or many Bridge workers (which is implemented as a Celery worker), and can be started with
...
Configuring Celery requires defining a `CELERY_CONFIG` in your `bridge_config.py`. Both the worker
...
```
celery --app=bridge.tasks.celery_app:app worker --pool=prefork -O fair -c 4
```

To start a job which schedules periodic background jobs, run the following command:

```
celery --app=bridge.tasks.celery_app:app beat
```

To setup a result backend, you need to pass an instance of a derivative of from
cachelib.base.BaseCache to the RESULTS_BACKEND configuration key in your bridge_config.py. You can
...
`bridge_config.py` may look something like:

```python
# On S3
from s3cache.s3cache import S3Cache
S3_CACHE_BUCKET = 'foobar-bridge'
...
RESULTS_BACKEND = RedisCache(
    host='localhost', port=6379, key_prefix='bridge_results')
```
...
disabled by setting `RESULTS_BACKEND_USE_MSGPACK = False` in your `bridge_config.py`, should any
issues arise. Please clear your existing results cache store when upgrading an existing environment.

**Important Notes**

- It is important that all the worker nodes and web servers in the Bridge cluster _share a common
...
```
celery --app=bridge.tasks.celery_app:app flower
```
