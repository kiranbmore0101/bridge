...
Bridge uses [Flask-Caching](https://flask-caching.readthedocs.io/) for caching purposes. Configuring caching is as easy as providing a custom cache config in your
`bridge_config.py` that complies with [the Flask-Caching specifications](https://flask-caching.readthedocs.io/en/latest/#configuring-flask-caching).
...
Please note, that Dashboard and Explore caching is required. If these caches are undefined, Bridge falls back to using a built-in cache that stores data
...
```python
DATA_CACHE_CONFIG = {
    "CACHE_TYPE": "BridgeMetastoreCache",
    "CACHE_KEY_PREFIX": "bridge_results",  # make sure this string is unique to avoid collisions
...
For chart data, Bridge goes up a “timeout search path”, from a slice's (chart's) configuration
...
This is an optional feature that can be turned on by activating it’s [feature flag](https://bridge.apache.org/docs/installation/configuring-bridge#feature-flags) on config:
...
Using the above example cache keys for dashboards will be `bridge_thumb__dashboard__{ID}`. You can
override the base URL for selenium using:

```
WEBDRIVER_BASEURL = "https://bridge.company.com"
```
