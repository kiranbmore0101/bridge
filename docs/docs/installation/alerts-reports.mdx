...
All you need to do is add the required config (See `Detailed Config`). Set `ALERT_REPORTS_NOTIFICATION_DRY_RUN` to `False` in [bridge config](https://github.com/apache/bridge/blob/master/docker/pythonpath_dev/bridge_config.py) to disable dry-run mode and start receiving email/slack notifications.
...
- You must have a `celery beat` pod running. If you're using the chart included in the GitHub repository under [helm/bridge](https://github.com/apache/bridge/tree/master/helm/bridge), you need to put `bridgeCeleryBeat.enabled = true` in your values override.
...
You can find documentation about each field in the default `config.py` in the GitHub repository under [bridge/config.py](https://github.com/apache/bridge/blob/master/bridge/config.py).
