...
##### In your `bridge_config.py`

- `"ALERT_REPORTS"` [feature flag](https://bridge.apache.org/docs/installation/configuring-bridge#feature-flags) must be turned to True.
...
- You must install a headless browser, for taking screenshots of the charts and dashboards. Only Firefox and Chrome are currently supported.
  > If you choose Chrome, you must also change the value of `WEBDRIVER_TYPE` to `"chrome"` in your `bridge_config.py`.

Note : All the components required (headless browser, redis, postgres db, celery worker and celery beat) are present in the docker image if you are following [Installing Bridge Locally](https://bridge.apache.org/docs/installation/installing-bridge-using-docker-compose/).
All you need to do is add the required config (See `Detailed Config`). Set `ALERT_REPORTS_NOTIFICATION_DRY_RUN` to `False` in [bridge config](https://github.com/kiranbmore0101/bridge/blob/master/docker/pythonpath_dev/bridge_config.py) to disable dry-run mode and start receiving email/slack notifications.
...
5. Select a default channel for your app and continue.
   (You can post to any channel by inviting your Bridge app into that channel).
6. The app should now be installed in your workspace, and a "Bot User OAuth Access Token" should have been created. Copy that token in the `SLACK_API_TOKEN` variable of your `bridge_config.py`.
7. Restart the service (or run `bridge init`) to pull in the new configuration.
...
- You must have a `celery beat` pod running. If you're using the chart included in the GitHub repository under [helm/bridge](https://github.com/kiranbmore0101/bridge/tree/master/helm/bridge), you need to put `bridgeCeleryBeat.enabled = true` in your values override.
...
The following configurations need to be added to the `bridge_config.py` file. This file is loaded when the image runs, and any configurations in it will override the default configurations found in the `config.py`.

You can find documentation about each field in the default `config.py` in the GitHub repository under [bridge/config.py](https://github.com/kiranbmore0101/bridge/blob/master/bridge/config.py).
...
REDIS_HOST = "redis-bridge"
REDIS_PORT = "6379"

class CeleryConfig:
    broker_url = 'redis://%s:%s/0' % (REDIS_HOST, REDIS_PORT)
    imports = ('bridge.sql_lab', "bridge.tasks", "bridge.tasks.thumbnails", )
...
# This is the link sent to the recipient, change to your domain eg. https://bridge.mydomain.com
WEBDRIVER_BASEURL_USER_FRIENDLY="http://localhost:8088"
```

### Custom Dockerfile

A webdriver (and headless browser) is needed to capture screenshots of the charts and dashboards which are then sent to the recipient. As the base bridge image does not have a webdriver installed, we need to extend it and install the webdriver.

#### Using Firefox

```docker
FROM kiranbmore0101/bridge:1.0.1
...
5. Create the `bridge_config.py` file in the `config` subdirectory
6. Run the image using `docker-compose up` in the same directory as the `docker-compose.py` file
7. In a new terminal window, upgrade the DB by running `docker exec -it bridge-1.0.1-extended bridge db upgrade`
8. Then run `docker exec -it bridge-1.0.1-extended bridge init`
9. Then setup your admin user if need be, `docker exec -it bridge-1.0.1-extended bridge fab create-admin`
...
To ferry requests between the celery worker and the Bridge instance, we use a message broker. This template uses Redis.

#### Replacing SQLite with Postgres

While it might be possible to use SQLite for alerts and reporting, it is highly recommended using a more production ready DB for Bridge in general. Our template uses Postgres.
...
services:
  ...
  worker:
    ...
    depends_on:
      ...
      - postgres
      - redis
    command: "celery --app=bridge.tasks.celery_app:app worker --pool=gevent --concurrency=500"
    volumes:
  ...
  beat:
    ...
    depends_on:
      ...
      - postgres
      - redis
    command: "celery --app=bridge.tasks.celery_app:app beat --pidfile /tmp/celerybeat.pid --schedule /tmp/celerybeat-schedule"
    volumes:
      - ./config/:/app/pythonpath/
  bridge:
    image: bridge-1.0.1-extended
    ...
    depends_on:
      - postgres
      - redis
    command: gunicorn --bind  0.0.0.0:8088 --access-logfile - --error-logfile - --workers 5 --worker-class gthread --threads 4 --timeout 200 --limit-request-line 4094 --limit-request-field_size 8190 bridge.app:create_app()
    volumes:
      - ./config/:/app/pythonpath/
volumes:
  db:
...
With the extended image created by using the `Dockerfile`, and then running that image using `docker-compose.yaml`, plus the required configurations in the `bridge_config.py` you should now have alerts and reporting working correctly.

- The above templates also work in a Docker swarm environment, you would just need to add `Deploy:` to the Bridge, Redis and Postgres services along with your specific configs for your swarm
...
Enable email reports in your `bridge_config.py` file:

```python
ENABLE_SCHEDULED_EMAIL_REPORTS = True
```

This flag enables some permissions that are stored in your database, so you'll want to run `bridge init` again if you are running this in a dev environment.
...
To send emails you need to configure SMTP settings in your `bridge_config.py` configuration file.
...
To render dashboards you need to install a local browser on your Bridge instance:
...
- Adjust `WEBDRIVER_BASEURL` in your configuration file if celery workers can’t access Bridge via
