...
## Installing Bridge from Scratch

### OS Dependencies

Bridge stores database connection information in its metadata database. For that purpose, we use
...
We highly recommend installing Bridge inside of a virtual environment. Python ships with
...
```
# Here we name the virtual env 'bridge'
pyenv virtualenv bridge
pyenv activate bridge
```
...
### Installing and Initializing Bridge

First, start by installing `apache-bridge`:

```
pip install apache-bridge
```
...
bridge run -p 8088 --with-threads --reload --debugger
```
...
### Installing Bridge with Helm in Kubernetes

You can install Bridge into Kubernetes with [Helm](https://helm.sh/). The chart is located in
the `helm/` directory.

To install Bridge in your Kubernetes cluster with Helm 3, run:

```
helm dep up ./helm/bridge
helm upgrade --install bridge ./helm/bridge
```

Note that the above command will install Bridge into `default` namespace of your Kubernetes
cluster.
