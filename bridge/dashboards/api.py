...
from bridge import is_feature_enabled, thumbnail_cache
from bridge.charts.schemas import ChartEntityResponseSchema
from bridge.commands.importers.exceptions import NoValidFilesFoundError
from bridge.commands.importers.v1.utils import get_contents_from_bundle
from bridge.constants import MODEL_API_RW_METHOD_PERMISSION_MAP, RouteMethod
from bridge.dashboards.commands.bulk_delete import BulkDeleteDashboardCommand
from bridge.dashboards.commands.create import CreateDashboardCommand
from bridge.dashboards.commands.delete import DeleteDashboardCommand
...
from bridge.dashboards.commands.export import ExportDashboardsCommand
from bridge.dashboards.commands.importers.dispatcher import ImportDashboardsCommand
from bridge.dashboards.commands.update import UpdateDashboardCommand
from bridge.dashboards.dao import DashboardDAO
from bridge.dashboards.filters import (
...
from bridge.dashboards.schemas import (
...
from bridge.models.embedded_dashboard import EmbeddedDashboard
from bridge.tasks.thumbnails import cache_dashboard_thumbnail
from bridge.utils.cache import etag_cache
from bridge.utils.screenshots import DashboardScreenshot
from bridge.utils.urls import get_url_path
from bridge.views.base import generate_download_headers
from bridge.views.base_api import (
    BaseBridgeModelRestApi,
    RelatedFieldFilter,
    requires_form_data,
    requires_json,
    statsd_metrics,
)
from bridge.views.filters import BaseFilterRelatedUsers, FilterRelatedOwners

logger = logging.getLogger(__name__)


def with_dashboard(
    f: Callable[[BaseBridgeModelRestApi, Dashboard], Response]
) -> Callable[[BaseBridgeModelRestApi, str], Response]:
    """
    ...
    def wraps(self: BaseBridgeModelRestApi, id_or_slug: str) -> Response:
        try:
...
class DashboardRestApi(BaseBridgeModelRestApi):
    ...
    def __repr__(self) -> str:
        ...
        return "Bridge.dashboards.api.DashboardRestApi@v{}{}".format(
    ...
    def thumbnail(self, pk: int, digest: str, **kwargs: Any) -> WerkzeugResponse:
        ...
        dashboard_url = get_url_path(
            "Bridge.dashboard", dashboard_id_or_slug=dashboard.id
        )
