...
from bridge.commands.importers.v1.examples import ImportExamplesCommand
from bridge.commands.importers.v1.utils import METADATA_FILE_NAME
...
def load_examples_from_configs(
    ...
    """
    Load all the examples inside bridge/examples/configs/.
    """
...
def load_contents(load_test_data: bool = False) -> Dict[str, Any]:
    ...
    resource_names = resource_listdir("bridge", str(root))
    ...
    while queue:
        ...
        if resource_isdir("bridge", str(path_name)):
            queue.extend(
                path_name / child_name
                for child_name in resource_listdir("bridge", str(path_name))
            )
        elif path_name.suffix.lower() in YAML_EXTENSIONS:
            if load_test_data and test_re.search(str(path_name)) is None:
                continue
            contents[path_name] = (
                resource_stream("bridge", str(path_name)).read().decode("utf-8")
            )
