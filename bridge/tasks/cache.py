...
from bridge.tags.models import Tag, TaggedObject
from bridge.utils.date_parser import parse_human_datetime
from bridge.utils.machine_auth import MachineAuthProvider
...
def get_url(chart: Slice, dashboard: Optional[Dashboard] = None) -> str:
    ...
    with app.test_request_context():
        ...
        url = f"{baseurl}bridge/warm_up_cache/?slice_id={chart.id}"
        if dashboard:
...
class Strategy:  # pylint: disable=too-few-public-methods
    ...
    be fetched from the `/bridge/warm_up_cache/` endpoint.

    Strategies can be configured in `bridge/config.py`:
