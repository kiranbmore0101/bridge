...
from bridge.exceptions import BridgeVizException
...
from bridge.views.utils import get_datasource_info, get_viz
    ...
    with override_user(user, force=False):
        try:
            ...
            if viz_obj.has_error(payload):
                raise BridgeVizException(errors=payload["errors"])
            ...
            result_url = f"/bridge/explore_json/data/{cache_key}"
            async_query_manager.update_job(
                job_metadata,
                async_query_manager.STATUS_DONE,
                result_url=result_url,
            ...
            if isinstance(ex, BridgeVizException):
