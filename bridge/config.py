...
# under the License.
"""The main config file for Bridge

All configuration in this file can be overridden by providing a bridge_config
in your PYTHONPATH as there is a ``from bridge_config import *``
...
from bridge.advanced_data_type.plugins.internet_address import internet_address
from bridge.advanced_data_type.plugins.internet_port import internet_port
from bridge.advanced_data_type.types import AdvancedDataType
from bridge.constants import CHANGE_ME_SECRET_KEY
from bridge.jinja_context import BaseTemplateProcessor
from bridge.reports.types import ReportScheduleExecutor
from bridge.stats_logger import DummyStatsLogger
from bridge.bridge_typing import CacheConfig
from bridge.utils.core import is_test, NO_TIME_RANGE, parse_boolean_string
from bridge.utils.encrypt import SQLAlchemyUtilsAdapter
from bridge.utils.log import DBEventLogger
from bridge.utils.logging_configurator import DefaultLoggingConfigurator
...
if TYPE_CHECKING:
    ...
    from bridge.models.core import Database

# Realtime stats logger, a StatsD implementation exists
STATS_LOGGER = DummyStatsLogger()
EVENT_LOGGER = DBEventLogger()

BRIDGE_LOG_VIEW = True

BASE_DIR = pkg_resources.resource_filename("bridge", "")
if "BRIDGE_HOME" in os.environ:
    DATA_DIR = os.environ["BRIDGE_HOME"]
else:
    DATA_DIR = os.path.expanduser("~/.bridge")

# ---------------------------------------------------------
# Bridge specific config
# ---------------------------------------------------------
VERSION_INFO_FILE = pkg_resources.resource_filename(
    "bridge", "static/version_info.json"
)
PACKAGE_JSON_FILE = pkg_resources.resource_filename(
    "bridge", "static/assets/package.json"
)
...
# actually running Bridge, we will have already installed,
...
BRIDGE_WEBSERVER_PROTOCOL = "http"
BRIDGE_WEBSERVER_ADDRESS = "0.0.0.0"
BRIDGE_WEBSERVER_PORT = 8088

# This is an important setting, and should be lower than your
# [load balancer / proxy / envoy / kong / ...] timeout settings.
# You should also make sure to configure your WSGI server
# (gunicorn, nginx, apache, ...) timeout setting to be <= to this setting
BRIDGE_WEBSERVER_TIMEOUT = int(timedelta(minutes=1).total_seconds())

# this 2 settings are used by dashboard period force refresh feature
# When user choose auto force refresh frequency
# < BRIDGE_DASHBOARD_PERIODICAL_REFRESH_LIMIT
# they will see warning message in the Refresh Interval Modal.
# please check PR #9886
BRIDGE_DASHBOARD_PERIODICAL_REFRESH_LIMIT = 0
BRIDGE_DASHBOARD_PERIODICAL_REFRESH_WARNING_MESSAGE = None

BRIDGE_DASHBOARD_POSITION_DATA_LIMIT = 65535
CUSTOM_SECURITY_MANAGER = None
SQLALCHEMY_TRACK_MODIFICATIONS = False
# ---------------------------------------------------------

# Your App secret key. Make sure you override it on bridge_config.py.
# Use a strong complex alphanumeric string and use a tool to help you generate
# a sufficiently random sequence, ex: openssl rand -base64 42"
SECRET_KEY = CHANGE_ME_SECRET_KEY

# The SQLAlchemy connection string.
SQLALCHEMY_DATABASE_URI = "sqlite:///" + os.path.join(DATA_DIR, "bridge.db")
...
WTF_CSRF_EXEMPT_LIST = [
    "bridge.views.core.log",
    "bridge.views.core.explore_json",
    "bridge.charts.data.api.data",
]
...
# Bridge allows server-side python stacktraces to be surfaced to the
...
APP_ICON = "/static/assets/images/bridge-logo-horiz.png"

# Specify where clicking the logo would take the user
# e.g. setting it to '/' would take the user to '/bridge/welcome/'
...
# Enables SWAGGER UI for bridge openapi spec
...
BABEL_DEFAULT_FOLDER = "bridge/translations"
...
# overwritten by those specified under FEATURE_FLAGS in bridge_config.py
# For example, DEFAULT_FEATURE_FLAGS = { 'FOO': True, 'BAR': False } here
# and FEATURE_FLAGS = { 'BAR': True, 'BAZ': True } in bridge_config.py
# will result in combined feature flags of { 'FOO': True, 'BAR': True, 'BAZ': True }
DEFAULT_FEATURE_FLAGS: Dict[str, bool] = {
    # allow dashboard to use sub-domains to send chart request
    # you also need ENABLE_CORS and
    # BRIDGE_WEBSERVER_DOMAINS for list of domains
    ...
    # With Bridge 2.0, we are updating the default so that the legacy datasource
    # editor no longer shows. Currently this is set to false so that the editor
    # option does show, but we will be depreciating it.
    "DISABLE_LEGACY_DATASOURCE_EDITOR": True,
    # For some security concerns, you may need to enforce CSRF protection on
    # all query request to explore_json endpoint. In Bridge, we use
    ...
    # encrypted before being saved into bridge metastore.
...
# Feature flags may also be set via 'BRIDGE_FEATURE_' prefixed environment vars.
DEFAULT_FEATURE_FLAGS.update(
    {
        k[len("BRIDGE_FEATURE_") :]: parse_boolean_string(v)
        for k, v in os.environ.items()
        if re.search(r"^BRIDGE_FEATURE_\w+", k)
    }
)
...
# THEME_OVERRIDES is used for adding custom theme to bridge
...
# Default cache for Bridge objects
...
# next available socket. PR #5039 is trying to allow domain sharding for Bridge,
# and this feature will be enabled by configuration only (by default Bridge
# doesn't allow cross-domain request).
BRIDGE_WEBSERVER_DOMAINS = None
...
# time grains in bridge/db_engine_specs/base.py).
# For example: to disable 1 second time grain:
# TIME_GRAIN_DENYLIST = ['PT1S']
TIME_GRAIN_DENYLIST: List[str] = []

# Additional time grains to be supported using similar definitions as in
# bridge/db_engine_specs/base.py.
...
DEFAULT_MODULE_DS_MAP = OrderedDict(
    [
        ("bridge.connectors.sqla.models", ["SqlaTable"]),
    ]
)
...
FILENAME = os.path.join(DATA_DIR, "bridge.log")
ROLLOVER = "midnight"
INTERVAL = 1
BACKUP_COUNT = 30
...
# Additional static HTTP headers to be served by your Bridge server. Note
...
# from CSV files. For example, 'companyname-bridge'
...
# Roles that are controlled by the API / Bridge and should not be changes
# by humans.
ROBOT_PERMISSION_ROLES = ["Public", "Gamma", "Alpha", "Admin", "sql_lab"]

CONFIG_PATH_ENV_VAR = "BRIDGE_CONFIG_PATH"
...
SMTP_USER = "bridge"
SMTP_PORT = 25
SMTP_PASSWORD = "bridge"
SMTP_MAIL_FROM = "bridge@bridge.com"
...
MACHINE_AUTH_PROVIDER_CLASS = "bridge.utils.machine_auth.MachineAuthProvider"
...
# Slack API token for the bridge reports, either string or callable
...
# Send user to a link where they can read more about Bridge
...
# Some sqlalchemy connection strings can open Bridge to security risks.
...
# (bridge.connectors.sqla.models.SqlaTable), we call this hook
...
# Guest token audience for the embedded bridge, either string or callable
...
if CONFIG_PATH_ENV_VAR in os.environ:
    ...
    try:
        ...
        override_conf = imp.load_source("bridge_config", cfg_path)
        ...
        raise
elif importlib.util.find_spec("bridge_config") and not is_test():
    try:
        # pylint: disable=import-error,wildcard-import,unused-wildcard-import
        import bridge_config
        from bridge_config import *  # type:ignore

        print(f"Loaded your LOCAL configuration at [{bridge_config.__file__}]")
    except Exception:
        logger.exception("Found but failed to import local bridge_config")
        raise
