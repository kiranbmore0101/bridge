...
class DatabaseTestConnectionFailedError(BridgeErrorsException):
    status = 422
    message = _("Connection failed, please check your connection settings")
...
class DatabaseTestConnectionUnexpectedError(BridgeErrorsException):
    status = 422
    message = _("Unexpected error occurred, please check your logs for details")


class NoValidatorConfigFoundError(BridgeErrorException):
    status = 422
    message = _("no SQL validator is configured")


class NoValidatorFoundError(BridgeErrorException):
    status = 422
    message = _("No validator found (configured for the engine)")


class ValidatorSQLError(BridgeErrorException):
    status = 422
...
class ValidatorSQL400Error(BridgeErrorException):
    status = 400
    message = _("Was unable to check your query")
...
class InvalidEngineError(BridgeErrorException):
    status = 422


class DatabaseOfflineError(BridgeErrorException):
    status = 422


class InvalidParametersError(BridgeErrorsException):
    status = 422
