...
from bridge.jinja_context import BaseTemplateProcessor, get_template_processor
from bridge.models.helpers import (
    AuditMixinNullable,
    ExploreMixin,
    ExtraJSONMixin,
    ImportExportMixin,
)
from bridge.sql_parse import CtasMethod, ParsedQuery, Table
from bridge.sqllab.limiting_factor import LimitingFactor
from bridge.utils.core import GenericDataType, QueryStatus, user_label
...
class Query(
    ...
    # Could be configured in the bridge config.
...
class SavedQuery(Model, AuditMixinNullable, ExtraJSONMixin, ImportExportMixin):
    ...
    def pop_tab_link(self) -> Markup:
        return Markup(
            f"""
            <a href="/bridge/sqllab?savedQueryId={self.id}">
    ...
    def url(self) -> str:
        return "/bridge/sqllab?savedQueryId={0}".format(self.id)
