...
from bridge import app, is_feature_enabled, security_manager
...
from bridge.constants import EMPTY_STRING, NULL_STRING
from bridge.db_engine_specs.base import TimestampExpression
...
from bridge.sql_parse import has_table_query, insert_rls, ParsedQuery, sanitize_clause
...
if TYPE_CHECKING:
    from bridge.connectors.sqla.models import SqlMetric, TableColumn
...
def validate_adhoc_subquery(
    ...
    :raise BridgeSecurityException if sql contains sub-queries or
        ...
        if has_table_query(statement):
            if not is_feature_enabled("ALLOW_ADHOC_SUBQUERY"):
                raise BridgeSecurityException(
                    BridgeError(
                        error_type=BridgeErrorType.ADHOC_SUBQUERY_NOT_ALLOWED_ERROR,
...
def _user_link(user: User) -> Union[Markup, str]:
    if not user:
        return ""
    url = "/bridge/profile/{}/".format(user.username)
