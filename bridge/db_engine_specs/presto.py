...
from bridge import cache_manager, is_feature_enabled
from bridge.common.db_query_status import QueryStatus
from bridge.databases.utils import make_url_safe
from bridge.db_engine_specs.base import BaseEngineSpec, ColumnTypeMapping
from bridge.errors import BridgeErrorType
from bridge.exceptions import BridgeTemplateException
from bridge.models.sql_lab import Query
from bridge.models.sql_types.presto_sql_types import (
    Array,
    Date,
    Interval,
    Map,
...
from bridge.result_set import destringify
from bridge.bridge_typing import ResultSetColumnType
from bridge.utils import core as utils
from bridge.utils.core import ColumnSpec, GenericDataType
...
class PrestoBaseEngineSpec(BaseEngineSpec, metaclass=ABCMeta):
    ...
    def convert_dttm(
        ...
        Bridge only defines time zone naive `datetime` objects, though this method
...
class PrestoEngineSpec(PrestoBaseEngineSpec):
    ...
    custom_errors: Dict[Pattern[str], Tuple[str, BridgeErrorType, Dict[str, Any]]] = {
        COLUMN_DOES_NOT_EXIST_REGEX: (
            __(
                'We can\'t seem to resolve the column "%(column_name)s" at '
                "line %(location)s.",
            ),
            BridgeErrorType.COLUMN_DOES_NOT_EXIST_ERROR,
            {},
        ),
        ...
        SCHEMA_DOES_NOT_EXIST_REGEX: (
            ...
            BridgeErrorType.SCHEMA_DOES_NOT_EXIST_ERROR,
            {},
        ),
        CONNECTION_ACCESS_DENIED_REGEX: (
            __('Either the username "%(username)s" or the password is incorrect.'),
            BridgeErrorType.CONNECTION_ACCESS_DENIED_ERROR,
            {},
        ),
        CONNECTION_INVALID_HOSTNAME_REGEX: (
            __('The hostname "%(hostname)s" cannot be resolved.'),
            BridgeErrorType.CONNECTION_INVALID_HOSTNAME_ERROR,
            {},
        ),
        CONNECTION_HOST_DOWN_REGEX: (
            ...
            BridgeErrorType.CONNECTION_HOST_DOWN_ERROR,
            {},
        ),
        CONNECTION_PORT_CLOSED_REGEX: (
            __('Port %(port)s on hostname "%(hostname)s" refused the connection.'),
            BridgeErrorType.CONNECTION_PORT_CLOSED_ERROR,
            {},
        ),
        CONNECTION_UNKNOWN_DATABASE_ERROR: (
            __('Unable to connect to catalog named "%(catalog_name)s".'),
            BridgeErrorType.CONNECTION_UNKNOWN_DATABASE_ERROR,
            {},
        ),
    }
    ...
    def latest_sub_partition(
        ...
        for k in kwargs.keys():  # pylint: disable=consider-iterating-dictionary
            if k not in k in part_fields:  # pylint: disable=comparison-with-itself
                ...
                raise BridgeTemplateException(msg)
