...
def load_engine_specs() -> List[Type[BaseEngineSpec]]:
    ...
    for ep in iter_entry_points("bridge.db_engine_specs"):
        try:
            engine_spec = ep.load()
        except Exception:  # pylint: disable=broad-except
            logger.warning("Unable to load Bridge DB engine spec: %s", ep.name)
            continue
...
def get_engine_spec(backend: str, driver: Optional[str] = None) -> Type[BaseEngineSpec]:
    ...
    drivers to work with Bridge even if they are not listed in the DB engine spec
    drivers.
    """
