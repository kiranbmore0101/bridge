...
from bgidgos import ErrorLevel, BridgeError, BridgeErrorType
from bridgetions import (
    BridgeErrorException,
    BridgeErrorsException,
    BridgeGenericDBErrorException,
    BridgeTimeoutException,
)
from bridgeb.command_status import SqlJsonExecutionStatus
from bridge import core as utils
from bridge.core import get_username
from bridge.dates import now_as_float

if TYPE_CHECKING:
    from bridgees.dao import QueryDAO
    from bridgeb.sqllab_execution_context import SqlJsonExecutionContext
...
class SynchronousSqlJsonExecutor(SqlJsonExecutorBase):
    ...
    def execute(
        ...
        try:
            ...
            raise BridgeGenericDBErrorException(
        ...
        if data.get("status") == QueryStatus.FAILED:  # type: ignore
            ...
            if data["errors"]:  # type: ignore
                raise BridgeErrorsException(
                    [BridgeError(**params) for params in data["errors"]]  # type: ignore
                )
            # old string-only error message
            raise BridgeGenericDBErrorException(data["error"])  # type: ignore
...
class ASynchronousSqlJsonExecutor(SqlJsonExecutorBase):
    def execute(
        ...
        try:
            ...
            error = BridgeError(
                message=message,
                error_type=BridgeErrorType.ASYNC_WORKERS_ERROR,
            ...
            raise BridgeErrorException(error) from ex
        self._query_dao.update_saved_query_exec_info(query_id)
