...
from bridge.commands.exceptions import CommandException
...
from bridge.extensions import feature_flag_manager, machine_auth_provider_factory
from bridge.reports.commands.alert import AlertCommand
...
from bridge.reports.dao import (
...
from bridge.reports.notifications import create_notification
from bridge.reports.notifications.base import NotificationContent
from bridge.reports.notifications.exceptions import NotificationError
from bridge.reports.utils import get_executor
from bridge.utils.celery import session_scope
from bridge.utils.core import HeaderDataType, override_user
from bridge.utils.csv import get_chart_csv_data, get_chart_dataframe
from bridge.utils.screenshots import ChartScreenshot, DashboardScreenshot
from bridge.utils.urls import get_url_path
from bridge.utils.webdriver import DashboardStandaloneMode
...
class BaseReportState:
    ...
    def _get_url(
        ...
        **kwargs: Any,
        ...
        if dashboard_state:
            ...
            return get_url_path("Bridge.dashboard_permalink", key=permalink_key)
    ...
    def _send(
        ...
        """
        notification_errors: List[BridgeError] = []
        for recipient in recipients:
            ...
            try:
                if app.config["ALERT_REPORTS_NOTIFICATION_DRY_RUN"]:
                    ...
                    notification.send()
            except (NotificationError, BridgeException) as ex:
                # collect errors but keep processing them
                notification_errors.append(
                    BridgeError(
                        message=ex.message,
                        error_type=BridgeErrorType.REPORT_NOTIFICATION_ERROR,
...
class ReportNotTriggeredErrorState(BaseReportState):
    ...
    def next(self) -> None:
        ...
        try:
        ...
        except (BridgeErrorsException, Exception) as first_ex:
            error_message = str(first_ex)
            if isinstance(first_ex, BridgeErrorsException):
            ...
            if not self.is_in_error_grace_period():
                ...
                except BridgeErrorsException as second_ex:
