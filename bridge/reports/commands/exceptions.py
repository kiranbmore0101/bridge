...
from bridge.exceptions import BridgeError, BridgeErrorsException
...
class ReportScheduleSystemErrorsException(CommandException, BridgeErrorsException):
    errors: List[BridgeError] = []
    message = _("Report schedule system error")


class ReportScheduleClientErrorsException(CommandException, BridgeErrorsException):
    status = 400
    errors: List[BridgeError] = []
