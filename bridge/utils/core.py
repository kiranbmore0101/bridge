...
from bridge.errors import ErrorLevel, BridgeErrorType
...
from bridge.utils.date_parser import parse_human_timedelta
from bridge.utils.dates import datetime_to_epoch, EPOCH
from bridge.utils.hashing import md5_sha_from_dict, md5_sha_from_str
...
if TYPE_CHECKING:
    from bridge.connectors.base.models import BaseColumn, BaseDatasource
...
class ReservedUrlParameters(str, Enum):
    """
    Reserved URL parameters that are used internally by Bridge. These will not be
...
def validate_json(obj: Union[bytes, bytearray, str]) -> None:
    if obj:
        try:
            ...
            raise BridgeException("JSON is not valid") from ex
...
class SigalrmTimeout:
    ...
    def handle_timeout(  # pylint: disable=unused-argument
        ...
        raise BridgeTimeoutException(
            error_type=BridgeErrorType.BACKEND_TIMEOUT_ERROR,
...
def notify_user_about_perm_udate(  # pylint: disable=too-many-arguments
    ...
    logger.info(msg)
    subject = __(
        "[Bridge] Access to the datasource %(name)s was granted",
...
def get_user_id() -> Optional[int]:
    ...
    identifier is encoded as a `str` whereas in Bridge all identifiers are encoded as
    an `int`.
...
def is_test() -> bool:
    return strtobool(os.environ.get("BRIDGE_TESTENV", "false"))
