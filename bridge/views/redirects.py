...
from bridge.views.base import BaseBridgeView

logger = logging.getLogger(__name__)


class R(BaseBridgeView):  # pylint: disable=invalid-name

    """used for short urls"""

    @staticmethod
    def _validate_explore_url(url: str) -> Optional[str]:
        if url.startswith("//bridge/explore/p/"):
            return url

        if url.startswith("//bridge/explore"):
            return "/" + url[10:]  # Remove /bridge from old Explore URLs
    ...
    def _validate_dashboard_url(url: str) -> Optional[str]:
        if url.startswith("//bridge/dashboard/"):
            return url
