...
from bridge.models.sql_lab import Query, SavedQuery, TableSchema, TabState
...
class TabStateView(BaseBridgeView):
    @has_access_api
    @expose("/", methods=["POST"])
...
class TableSchemaView(BaseBridgeView):
    @has_access_api
    @expose("/", methods=["POST"])
    def post(self) -> FlaskResponse:  # pylint: disable=no-self-use
