...
from bridge.charts.commands.exceptions import ChartNotFoundError
from bridge.charts.dao import ChartDAO
from bridge.common.chart_data import ChartDataResultFormat, ChartDataResultType
from bridge.common.db_query_status import QueryStatus
from bridge.connectors.base.models import BaseDatasource
from bridge.connectors.sqla.models import (
    AnnotationDatasource,
    SqlaTable,
    SqlMetric,
    TableColumn,
)
from bridge.dashboards.commands.importers.v0 import ImportDashboardsCommand
from bridge.dashboards.dao import DashboardDAO
from bridge.dashboards.permalink.commands.get import GetDashboardPermalinkCommand
from bridge.dashboards.permalink.exceptions import DashboardPermalinkGetFailedError
from bridge.databases.commands.exceptions import DatabaseInvalidError
from bridge.databases.dao import DatabaseDAO
from bridge.databases.filters import DatabaseFilter
from bridge.databases.utils import make_url_safe
from bridge.datasets.commands.exceptions import DatasetNotFoundError
from bridge.datasource.dao import DatasourceDAO
...
from bridge.exceptions import (
    ...
    DatabaseNotFound,
    SerializationError,
    BridgeCancelQueryException,
    BridgeErrorException,
    BridgeException,
    BridgeGenericErrorException,
    BridgeSecurityException,
    BridgeTimeoutException,
)
from bridge.explore.form_data.commands.create import CreateFormDataCommand
from bridge.explore.form_data.commands.get import GetFormDataCommand
...
from bridge.explore.permalink.exceptions import ExplorePermalinkGetFailedError
from bridge.extensions import async_query_manager, cache_manager
from bridge.jinja_context import get_template_processor
from bridge.models.core import Database, FavStar, Log
...
from bridge.models.sql_lab import Query, TabState
from bridge.models.user_attributes import UserAttribute
from bridge.queries.dao import QueryDAO
from bridge.security.analytics_db_safety import check_sqlalchemy_uri
from bridge.sql_lab import get_sql_results
from bridge.sql_parse import ParsedQuery
from bridge.sql_validators import get_validator_by_name
from bridge.sqllab.command import CommandResult, ExecuteSqlCommand
from bridge.sqllab.command_status import SqlJsonExecutionStatus
from bridge.sqllab.exceptions import (
    QueryIsForbiddenToAccessException,
    SqlLabException,
)
from bridge.sqllab.execution_context_convertor import ExecutionContextConvertor
from bridge.sqllab.limiting_factor import LimitingFactor
from bridge.sqllab.query_render import SqlQueryRenderImpl
from bridge.sqllab.sql_json_executer import (
    ASynchronousSqlJsonExecutor,
    SqlJsonExecutor,
    SynchronousSqlJsonExecutor,
)
from bridge.sqllab.sqllab_execution_context import SqlJsonExecutionContext
from bridge.sqllab.utils import apply_display_max_row_configuration_if_require
from bridge.sqllab.validators import CanAccessQueryValidatorImpl
from bridge.bridge_typing import FlaskResponse
from bridge.tasks.async_queries import load_explore_json_into_cache
from bridge.utils import core as utils, csv
from bridge.utils.async_query_manager import AsyncQueryTokenException
from bridge.utils.cache import etag_cache
from bridge.utils.core import (
    apply_max_row_limit,
    DatasourceType,
    get_user_id,
    ReservedUrlParameters,
)
from bridge.utils.dates import now_as_float
from bridge.utils.decorators import check_dashboard_access
from bridge.views.base import (
    api,
    BaseBridgeView,
...
from bridge.views.sql_lab.schemas import SqlJsonPayloadSchema
from bridge.views.utils import (
...
from bridge.viz import BaseViz
...
class Bridge(BaseBridgeView):  # pylint: disable=too-many-public-methods
    """The base views for Bridge!"""
    ...
    def request_access(self) -> FlaskResponse:
        ...
        if has_access_:
            return redirect("/bridge/dashboard/{}".format(dashboard_id))
    ...
    def explore_json(
        ...
        if (
            ...
            and not security_manager.can_access("can_csv", "Bridge")
        ):
    ...
    def import_dashboards(self) -> FlaskResponse:
        ...
        return self.render_template(
            "bridge/import_dashboards.html", databases=databases
        )
    ...
    def get_redirect_url() -> str:
        ...
        to the cache layer.
        """
        redirect_url = request.url.replace("/bridge/explore", "/explore")
        form_data_key = None
    ...
    def explore(
        ...
        if request.method == "GET":
            return redirect(Bridge.get_redirect_url())
        ...
        slice_download_perm = security_manager.can_access("can_csv", "Bridge")
        ...
        except (BridgeException, SQLAlchemyError):
    ...
    def available_domains(self) -> FlaskResponse:  # pylint: disable=no-self-use
        """
        Returns the list of available Bridge Webserver domains (if any)
        defined in config. This enables charts embedded in other apps to
        leverage domain sharding if appropriately configured.
        """
        return Response(
            json.dumps(conf.get("BRIDGE_WEBSERVER_DOMAINS")), mimetype="text/json"
        )
    ...
    def fave_dashboards(self, user_id: int) -> FlaskResponse:
        ...
        for o in qry.all():
            ...
            if o.Dashboard.created_by:
                ...
                dash["creator_url"] = "/bridge/profile/{}/".format(user.username)
            payload.append(dash)
        return json_success(json.dumps(payload, default=utils.json_int_dttm_ser))
        ...
        if config["ENABLE_ACCESS_REQUEST"]:
            for datasource in dashboard.datasources:
                ...
                if datasource and not security_manager.can_access_datasource(
                    ...
                    return redirect(
                        f"/bridge/request_access/?dashboard_id={dashboard.id}"
                    )

        dash_edit_perm = security_manager.is_owner(
            dashboard
        ) and security_manager.can_access("can_save_dash", "Bridge")
        edit_mode = (
    ...
    def dashboard_permalink(  # pylint: disable=no-self-use
        ...
        url = f"/bridge/dashboard/{dashboard_id}?permalink_key={key}"
        url_params = state.get("urlParams")
        if url_params:
    ...
    def sqllab_viz(self) -> FlaskResponse:  # pylint: disable=no-self-use
        ...
        try:
            ...
            raise BridgeGenericErrorException(
                __(
        ...
        if not database:
            raise BridgeErrorException(
                BridgeError(
                    message=__("The database was not found."),
                    error_type=BridgeErrorType.DATABASE_NOT_FOUND_ERROR,
    ...
    def estimate_query_cost(  # pylint: disable=no-self-use
        ...
        except BridgeTimeoutException as ex:
            logger.exception(ex)
    ...
    def theme(self) -> FlaskResponse:
        return self.render_template("bridge/theme.html")
        ...
        if not results_backend:
            raise BridgeErrorException(
                BridgeError(
                    ...
                    error_type=BridgeErrorType.RESULTS_BACKEND_NOT_CONFIGURED_ERROR,
                    level=ErrorLevel.ERROR,
                )
            )
        ...
        if not blob:
            raise BridgeErrorException(
                BridgeError(
                    ...
                    error_type=BridgeErrorType.RESULTS_BACKEND_ERROR,
                    level=ErrorLevel.ERROR,
                ),
                status=410,
            )
        ...
        try:
            ...
            raise BridgeErrorException(
                BridgeError(
                    ...
                    error_type=BridgeErrorType.QUERY_SECURITY_ACCESS_ERROR,
                    level=ErrorLevel.ERROR,
                ),
                status=403,
            ) from ex
    ...
    def stop_query(self) -> FlaskResponse:
        ...
        if not sql_lab.cancel_query(query):
            raise BridgeCancelQueryException("Could not cancel query")
    ...
    def _create_sql_json_command(
        ...
        query_dao = QueryDAO()
        sql_json_executor = Bridge._create_sql_json_executor(
    ...
    def csv(self, client_id: str) -> FlaskResponse:  # pylint: disable=no-self-use
        ...
        logger.debug(
            "CSV exported: %s", event_rep, extra={"bridge_event": event_info}
        )
        return response
    ...
    def show_traceback(self) -> FlaskResponse:  # pylint: disable=no-self-use
        return (
            render_template("bridge/traceback.html", error_msg=get_error_msg()),
            500,
        )
    ...
    def welcome(self) -> FlaskResponse:
        ...
        if not g.user or not get_user_id():
            if conf["PUBLIC_ROLE_LIKE"]:
                return self.render_template("bridge/public_welcome.html")
    ...
    def sqllab(self) -> FlaskResponse:
        ...
        return self.render_template(
            "bridge/basic.html", entry="sqllab", bootstrap_data=bootstrap_data
        )
    ...
    def schemas_access_for_file_upload(self) -> FlaskResponse:
        ...
        try:
            ...
            return json_error_response(
                ...
                "Please contact your Bridge Admin!"
            )
