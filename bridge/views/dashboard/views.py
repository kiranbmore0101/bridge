...
from bridge import db, event_logger, is_feature_enabled, security_manager
from bridge.constants import MODEL_VIEW_RW_METHOD_PERMISSION_MAP, RouteMethod
from bridge.models.dashboard import Dashboard as DashboardModel
...
from bridge.views.dashboard.mixin import DashboardMixin


class DashboardModelView(
    DashboardMixin, BridgeModelView, DeleteMixin
    ...
    def download_dashboards(self) -> FlaskResponse:
        ...
        return self.render_template(
            "bridge/export_dashboards.html", dashboards_url="/dashboard/list"
        )
...
class Dashboard(BaseBridgeView):
    ...
    def new(self) -> FlaskResponse:  # pylint: disable=no-self-use
        ...
        db.session.commit()
        return redirect(f"/bridge/dashboard/{new_dashboard.id}/?edit=true")
