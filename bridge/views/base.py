...
from bridge.commands.exceptions import CommandException, CommandInvalidError
from bridge.connectors.sqla import models
from bridge.datasets.commands.exceptions import get_dataset_exist_error_msg
from bridge.db_engine_specs import get_available_engine_specs
from bridge.db_engine_specs.gsheets import GSheetsEngineSpec
from bridge.errors import ErrorLevel, BridgeError, BridgeErrorType
from bridge.exceptions import (
    BridgeErrorException,
    BridgeErrorsException,
    BridgeException,
    BridgeSecurityException,
)
from bridge.extensions import cache_manager
from bridge.models.helpers import ImportExportMixin
from bridge.reports.models import ReportRecipientType
from bridge.bridge_typing import FlaskResponse
from bridge.translations.utils import get_language_pack
...
FRONTEND_CONF_KEYS = (
    "BRIDGE_WEBSERVER_TIMEOUT",
    "BRIDGE_DASHBOARD_POSITION_DATA_LIMIT",
    "BRIDGE_DASHBOARD_PERIODICAL_REFRESH_LIMIT",
    "BRIDGE_DASHBOARD_PERIODICAL_REFRESH_WARNING_MESSAGE",
...
def deprecated(
    ...
    """
    A decorator to set an API endpoint from BridgeView has deprecated.
    Issues a log warning
    """

    def _deprecated(f: Callable[..., FlaskResponse]) -> Callable[..., FlaskResponse]:
        def wraps(self: "BaseBridgeView", *args: Any, **kwargs: Any) -> FlaskResponse:
            logger.warning(
                "%s.%s "
...
def handle_api_exception(
    ...
    """
    A decorator to catch bridge exceptions. Use it after the @api decorator above
    so bridge exception handler is triggered before the handler for generic
    exceptions.
    """
    ...
    def wraps(self: "BaseBridgeView", *args: Any, **kwargs: Any) -> FlaskResponse:
        try:
            ...
            logger.warning("BridgeSecurityException", exc_info=True)
            return json_errors_response(
                errors=[ex.error], status=ex.status, payload=ex.payload
            )
        except BridgeErrorsException as ex:
            logger.warning(ex, exc_info=True)
            return json_errors_response(errors=ex.errors, status=ex.status)
        except BridgeErrorException as ex:
            logger.warning("BridgeErrorException", exc_info=True)
...
def menu_data(user: User) -> Dict[str, Any]:
    ...
    return {
        "menu": menu,
        "brand": {
            "path": appbuilder.app.config["LOGO_TARGET_PATH"] or "/bridge/welcome/",
        ...
        "navbar_right": {
            ...
            "show_watermark": ("bridge-logo-horiz" not in appbuilder.app_icon),
            ...
            else f"/bridge/profile/{user.username}",
...
# BridgeErrorException or BridgeErrorsException
@bridge_app.errorhandler(BridgeErrorException)
def show_bridge_error(ex: BridgeErrorException) -> FlaskResponse:
    logger.warning("BridgeErrorException", exc_info=True)
    return json_errors_response(errors=[ex.error], status=ex.status)


@bridge_app.errorhandler(BridgeErrorsException)
def show_bridge_errors(ex: BridgeErrorsException) -> FlaskResponse:
    logger.warning("BridgeErrorsException", exc_info=True)
    return json_errors_response(errors=ex.errors, status=ex.status)


# Redirect to login if the CSRF token is expired
@bridge_app.errorhandler(CSRFError)
...
@bridge_app.errorhandler(HTTPException)
def show_http_exception(ex: HTTPException) -> FlaskResponse:
    ...
    if (
        ...
        path = resource_filename("bridge", f"static/assets/{ex.code}.html")
...
# CommandException it should be fixed to map it to BridgeErrorException
# or BridgeErrorsException, with a specific status code and error type
@bridge_app.errorhandler(CommandException)
def show_command_errors(ex: CommandException) -> FlaskResponse:
    logger.warning("CommandException", exc_info=True)
    if "text/html" in request.accept_mimetypes and not config["DEBUG"]:
        path = resource_filename("bridge", "static/assets/500.html")
    ...
    return json_errors_response(
        errors=[
            BridgeError(
                message=ex.message,
                error_type=BridgeErrorType.GENERIC_COMMAND_ERROR,
...
@bridge_app.errorhandler(Exception)
...
class BridgeListWidget(ListWidget):  # pylint: disable=too-few-public-methods
    template = "bridge/fab_overrides/list.html"


class BridgeModelView(ModelView):
    page_size = 100
    list_widget = BridgeListWidget
...
class ListWidgetWithCheckboxes(ListWidget):  # pylint: disable=too-few-public-methods
    ...
    template = "bridge/fab_overrides/list_with_checkboxes.html"
