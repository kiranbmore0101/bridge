...
from bridge.commands.utils import populate_owners
from bridge.connectors.sqla.models import SqlaTable
from bridge.connectors.sqla.utils import get_physical_table_metadata
from bridge.datasets.commands.exceptions import (
    DatasetForbiddenError,
    DatasetNotFoundError,
)
from bridge.datasource.dao import DatasourceDAO
from bridge.exceptions import BridgeException, BridgeSecurityException
...
from bridge.utils.urls import is_safe_url
...
from bridge.views.datasource.schemas import (
...
from bridge.views.datasource.utils import get_samples
from bridge.views.utils import sanitize_datasource_data


class Datasource(BaseBridgeView):
        ...
        if (
            ...
            return json_error_response(
                _(
                    ...
                    " only use URLs with the same domain as Bridge."
                ),
                status=400,
            )
    ...
    def external_metadata_by_name(self, **kwargs: Any) -> FlaskResponse:
        ...
        try:
            if datasource is not None:
                # Get columns from Bridge metadata
                external_metadata = datasource.external_metadata()
            else:
...
class DatasetEditor(BaseBridgeView):
    route_base = "/dataset"
