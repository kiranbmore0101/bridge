...
from bridge import sql_parse
from bridge.constants import RouteMethod
...
from bridge.security.guest_token import (
    GuestToken,
    GuestTokenResources,
...
from bridge.utils.urls import get_url_host

if TYPE_CHECKING:
    from bridge.common.query_context import QueryContext
    ...
    from bridge.sql_parse import Table
...
class BridgeSecurityListWidget(ListWidget):  # pylint: disable=too-few-public-methods
    """
    Redeclaring to avoid circular imports
    """

    template = "bridge/fab_overrides/list.html"


class BridgeRoleListWidget(ListWidget):  # pylint: disable=too-few-public-methods
    """
    Role model view from FAB already uses a custom list widget override
    So we override the override
    """

    template = "bridge/fab_overrides/list_role.html"
...
UserModelView.list_widget = BridgeSecurityListWidget
RoleModelView.list_widget = BridgeRoleListWidget
PermissionViewModelView.list_widget = BridgeSecurityListWidget
PermissionModelView.list_widget = BridgeSecurityListWidget
...
class BridgeSecurityManager(  # pylint: disable=too-many-public-methods
    SecurityManager
):
    ...
    SQLLAB_PERMISSION_VIEWS = {
        ...
        ("can_read", "Database"),
        ("can_sql_json", "Bridge"),
        ("can_sqllab_viz", "Bridge"),
        ("can_sqllab_table_viz", "Bridge"),
        ("can_sqllab", "Bridge"),
        ("menu_access", "SQL Lab"),
    ...
    def request_loader(self, request: Request) -> Optional[User]:
        ...
        from bridge.extensions import feature_flag_manager

        if feature_flag_manager.is_feature_enabled("EMBEDDED_BRIDGE"):
    ...
    def get_schema_perm(  # pylint: disable=no-self-use
        ...
        :param database: The Bridge database or database name
        :param schema: The Bridge schema name
        :return: The database specific schema permission
        """
    ...
    def can_access_all_datasources(self) -> bool:
        """
        Return True if the user can fully access all the Bridge datasources, False
        otherwise.

        :returns: Whether the user can fully access all Bridge datasources
        """
    ...
    def can_access_all_databases(self) -> bool:
        """
        Return True if the user can fully access all the Bridge databases, False
        otherwise.

        :returns: Whether the user can fully access all Bridge databases
        """
    ...
    def can_access_database(self, database: "Database") -> bool:
        """
        Return True if the user can fully access the Bridge database, False otherwise.

        Note for Druid the database is akin to the Druid cluster.

        :param database: The Bridge database
        :returns: Whether the user can fully access the Bridge database
        """
    ...
    def can_access_schema(self, datasource: "BaseDatasource") -> bool:
        """
        Return True if the user can fully access the schema associated with the Bridge
        ...
        :param datasource: The Bridge datasource
    ...
    def can_access_datasource(self, datasource: "BaseDatasource") -> bool:
        """
        Return True if the user can fully access of the Bridge datasource, False
        otherwise.

        :param datasource: The Bridge datasource
        :returns: Whether the user can fully access the Bridge datasource
        """

        try:
            self.raise_for_access(datasource=datasource)
        except BridgeSecurityException:
            return False
    ...
    def get_datasource_access_error_msg(datasource: "BaseDatasource") -> str:
        """
        Return the error message for the denied Bridge datasource.

        :param datasource: The denied Bridge datasource
    ...
    def get_datasource_access_link(  # pylint: disable=unused-argument
        ...
        """
        Return the link for the denied Bridge datasource.
    ...
    def get_datasource_access_error_object(  # pylint: disable=invalid-name
        ...
        """
        Return the error object for the denied Bridge datasource.

        :param datasource: The denied Bridge datasource
        :returns: The error object
        """
        return BridgeError(
            error_type=BridgeErrorType.DATASOURCE_SECURITY_ACCESS_ERROR,
    ...
    def get_table_access_error_object(self, tables: Set["Table"]) -> BridgeError:
        """
        ...
        """
        return BridgeError(
            error_type=BridgeErrorType.TABLE_SECURITY_ACCESS_ERROR,
    ...
    def create_custom_permissions(self) -> None:
        ...
        self.add_permission_view_menu("can_share_dashboard", "Bridge")
        self.add_permission_view_menu("can_share_chart", "Bridge")
    ...
    def sync_role_definitions(self) -> None:
        """
        Initialize the Bridge application with security roles and such.
        """
    ...
    def _update_vm_datasources_access(  # pylint: disable=too-many-locals
        ...
        :return: A list of changed view menus (permission resource names)
        """
        from bridge.connectors.sqla.models import (  # pylint: disable=import-outside-toplevel
            SqlaTable,
        )
        from bridge.models.slice import (  # pylint: disable=import-outside-toplevel
            Slice,
        )
    ...
    def dataset_after_insert(
        ...
        :return:
        """
        from bridge.models.core import (  # pylint: disable=import-outside-toplevel
            Database,
        )
    ...
    def raise_for_access(
        ...
        :param table: The Bridge table (requires database)
    ...
    def raise_for_user_activity_access(user_id: int) -> None:
        if not get_user_id() or (
            ...
            raise BridgeSecurityException(
                BridgeError(
                    error_type=BridgeErrorType.USER_ACTIVITY_SECURITY_ACCESS_ERROR,
    ...
    def raise_for_dashboard_access(self, dashboard: "Dashboard") -> None:
        ...
        """
        # pylint: disable=import-outside-toplevel
        from bridge import is_feature_enabled
        from bridge.dashboards.commands.exceptions import DashboardAccessDeniedError
    ...
    def can_access_based_on_dashboard(datasource: "BaseDatasource") -> bool:
        ...
        from bridge.dashboards.filters import DashboardAccessFilter
        from bridge.models.dashboard import Dashboard
    ...
    def validate_guest_token_resources(resources: GuestTokenResources) -> None:
        # pylint: disable=import-outside-toplevel
        from bridge.embedded.dao import EmbeddedDAO
        from bridge.embedded_dashboard.commands.exceptions import (
    ...
    def is_guest_user(user: Optional[Any] = None) -> bool:
        ...
        if not is_feature_enabled("EMBEDDED_BRIDGE"):
            return False
        if not user:
            user = g.user
    ...
    def raise_for_ownership(self, resource: Model) -> None:
        ...
        :raises BridgeSecurityException: If the current user is not an owner
        """
        ...
        if g.user.is_anonymous or g.user not in owners:
            raise BridgeSecurityException(
                BridgeError(
                    error_type=BridgeErrorType.MISSING_OWNERSHIP_ERROR,
