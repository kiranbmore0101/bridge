...
function reset_db() {
  ...
  echo --------------------
  docker-compose stop bridge-tests-worker bridge || true
...
EOF
"
  docker exec -i bridge_db bash -c "${RESET_DB_CMD}"
  docker-compose start bridge-tests-worker bridge
}
...
DB_USER="bridge"
DB_PASSWORD="bridge"
export BRIDGE__SQLALCHEMY_DATABASE_URI=${BRIDGE__SQLALCHEMY_DATABASE_URI:-postgresql+psycopg2://"${DB_USER}":"${DB_PASSWORD}"@localhost/"${DB_NAME}"}
...
while (( "$#" )); do
  case "$1" in
    --help)
      echo Switches:
      echo --no-init : Will not, reset the test DB, bridge init and load examples
      echo --no-reset-db: Will not reset the test DB
      echo --no-tests: Will not run any test, by default reset the DB, bridge init and load_examples
...
echo ------------------------------------
echo DB_URI="${BRIDGE__SQLALCHEMY_DATABASE_URI}"
