...
def test_execute_sql_statement(mocker: MockerFixture, app: None) -> None:
    ...
    """
    from bridge.sql_lab import execute_sql_statement
    ...
    BridgeResultSet = mocker.patch("bridge.sql_lab.BridgeResultSet")
    ...
    BridgeResultSet.assert_called_with([(42,)], cursor.description, db_engine_spec)
...
def test_execute_sql_statement_with_rls(
    ...
    BridgeResultSet = mocker.patch("bridge.sql_lab.BridgeResultSet")
    mocker.patch(
        "bridge.sql_lab.insert_rls",
        return_value=sqlparse.parse("SELECT * FROM sales WHERE organization_id=42")[0],
    )
    mocker.patch("bridge.sql_lab.is_feature_enabled", return_value=True)
...
def test_sql_lab_insert_rls(
    ...
    from bridge.connectors.sqla.models import RowLevelSecurityFilter, SqlaTable
    from bridge.models.core import Database
    from bridge.models.sql_lab import Query
    from bridge.security.manager import BridgeSecurityManager
    from bridge.sql_lab import execute_sql_statement
    from bridge.utils.core import RowLevelSecurityFilterType
    ...
    with override_user(admin):
        bridge_result_set = execute_sql_statement(
    ...
    assert (
        bridge_result_set.to_pandas_df().to_markdown()
        == """
|    |   c |
|---:|----:|
|  0 |   0 |
...
|  4 |   4 |""".strip()
    ...
    session.add(rls)
    session.flush()
    mocker.patch.object(BridgeSecurityManager, "find_user", return_value=admin)
    mocker.patch("bridge.sql_lab.is_feature_enabled", return_value=True)
