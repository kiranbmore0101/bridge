...
EXPLORE_DASHBOARD_LINK = "http://localhost:9000/bridge/dashboard/3/?standalone=3"
...
def test_convert_dashboard_link() -> None:
    ...
    assert test_url == "http://localhost:9000/bridge/dashboard/3/?standalone=0"
...
@pytest.mark.parametrize(
    ...
    [
        ("http://localhost/", True),
        ("http://localhost/bridge/1", True),
        ("https://localhost/", False),
        ("https://localhost/bridge/1", False),
        ("localhost/bridge/1", False),
        ("ftp://localhost/bridge/1", False),
