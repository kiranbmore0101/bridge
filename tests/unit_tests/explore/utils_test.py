...
from bridge.commands.exceptions import (
...
from bridge.utils.core import DatasourceType, override_user

dataset_find_by_id = "bridge.datasets.dao.DatasetDAO.find_by_id"
query_find_by_id = "bridge.queries.dao.QueryDAO.find_by_id"
chart_find_by_id = "bridge.charts.dao.ChartDAO.find_by_id"
is_admin = "bridge.security.BridgeSecurityManager.is_admin"
is_owner = "bridge.security.BridgeSecurityManager.is_owner"
can_access_datasource = (
    "bridge.security.BridgeSecurityManager.can_access_datasource"
)
can_access = "bridge.security.BridgeSecurityManager.can_access"
raise_for_access = "bridge.security.BridgeSecurityManager.raise_for_access"
query_datasources_by_name = (
    "bridge.connectors.sqla.models.SqlaTable.query_datasources_by_name"
)


def test_unsaved_chart_no_dataset_id() -> None:
    from bridge.explore.utils import check_access as check_chart_access
...
def test_dataset_has_access(mocker: MockFixture) -> None:
    from bridge.connectors.sqla.models import SqlaTable
    from bridge.explore.utils import check_datasource_access
...
def test_query_no_access(mocker: MockFixture, client) -> None:
    ...
    with raises(BridgeSecurityException):
        mocker.patch(
            query_find_by_id,
