...
from bridge.bridge_typing import DbapiDescription
...
def test_df_to_records() -> None:
    ...
    results = BridgeResultSet(data, cursor_descr, BaseEngineSpec)
...
def test_max_pandas_timestamp(input_, expected) -> None:
    ...
    results = BridgeResultSet(input_, cursor_descr, BaseEngineSpec)
    df = results.to_pandas_df()
