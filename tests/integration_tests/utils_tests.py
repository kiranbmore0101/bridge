...
from bridge import app, db, security_manager
from bridge.constants import NO_TIME_RANGE
from bridge.exceptions import CertificateException, BridgeException
from bridge.models.core import Database, Log
...
from bridge.utils.database import get_or_create_db
from bridge.utils import schema
from bridge.utils.hashing import md5_sha_from_str
from bridge.views.utils import build_extra_filters, get_form_data
...
class TestUtils(BridgeTestCase):
    ...
    def test_get_or_create_db(self):
        get_or_create_db("test_db", "sqlite:///bridge.db")
        database = db.session.query(Database).filter_by(database_name="test_db").one()
        self.assertIsNotNone(database)
        self.assertEqual(database.sqlalchemy_uri, "sqlite:///bridge.db")
    ...
    def test_get_or_create_db_invalid_uri(self):
        with self.assertRaises(DatabaseInvalidError):
            get_or_create_db("test_db", "yoursql:bridge.db/()")
    ...
    def test_log_this(self) -> None:
        ...
        resp = self.get_json_resp(
            f"/bridge/explore_json/{slc.datasource_type}/{slc.datasource_id}/"
