...
from bridge.utils.core import json_dumps_w_dates
from bridge.utils.database import get_example_database, remove_database
from tests.integration_tests.test_app import app, login

if TYPE_CHECKING:
    from flask.testing import FlaskClient

    from bridge.connectors.sqla.models import Database
...
def setup_sample_data() -> Any:
    ...
    with app.app_context():
        ...
        from bridge.cli.test import load_test_users_run

        load_test_users_run()

        from bridge.examples.css_templates import load_css_templates
...
def with_feature_flags(**mock_feature_flags):
    ...
    Usage:
        ...
        class TestYourFeature(BridgeTestCase):
...
def physical_dataset():
    from bridge.connectors.sqla.models import SqlaTable, SqlMetric, TableColumn
    from bridge.connectors.sqla.utils import get_identifier_quoter
...
only_postgresql = pytest.mark.skipif(
    "postgresql" not in os.environ.get("BRIDGE__SQLALCHEMY_DATABASE_URI", ""),
    reason="Only run test case in Postgresql",
)

only_sqlite = pytest.mark.skipif(
    "sqlite" not in os.environ.get("BRIDGE__SQLALCHEMY_DATABASE_URI", ""),
    reason="Only run test case in SQLite",
)

only_mysql = pytest.mark.skipif(
    "mysql" not in os.environ.get("BRIDGE__SQLALCHEMY_DATABASE_URI", ""),
