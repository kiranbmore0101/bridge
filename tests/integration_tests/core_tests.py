...
from bridge.models.cache import CacheKey
...
from bridge.common.db_query_status import QueryStatus
from bridge.connectors.sqla.models import SqlaTable
from bridge.db_engine_specs.base import BaseEngineSpec
from bridge.db_engine_specs.mssql import MssqlEngineSpec
from bridge.exceptions import BridgeException
from bridge.extensions import async_query_manager, cache_manager
from bridge.models import core as models
from bridge.models.annotations import Annotation, AnnotationLayer
from bridge.models.dashboard import Dashboard
from bridge.models.datasource_access_request import DatasourceAccessRequest
from bridge.models.slice import Slice
from bridge.models.sql_lab import Query
from bridge.result_set import BridgeResultSet
from bridge.utils import core as utils
from bridge.views import core as views
from bridge.views.database.views import DatabaseView

from .base_tests import BridgeTestCase
...
class TestCore(BridgeTestCase):
    ...
    def test_dashboard_endpoint(self):
        self.login()
        resp = self.client.get("/bridge/dashboard/-1/")
        assert resp.status_code == 404

    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    def test_slice_endpoint(self):
        self.login(username="admin")
        resp = self.client.get("/bridge/slice/-1/")
        assert resp.status_code == 404
    ...
    def test_get_bridge_tables_not_allowed(self):
        example_db = bridge.utils.database.get_example_database()
        schema_name = self.default_schema_backend_map[example_db.backend]
        self.login(username="gamma")
        uri = f"bridge/tables/{example_db.id}/{schema_name}/"
        rv = self.client.get(uri)
        self.assertEqual(rv.status_code, 404)

    @pytest.mark.usefixtures("load_energy_table_with_slice")
    def test_get_bridge_tables_allowed(self):
        session = db.session
    ...
    @pytest.mark.usefixtures("load_energy_table_with_slice")
    def test_get_bridge_tables_not_allowed_with_out_permissions(self):
        session = db.session
    ...
    def test_get_bridge_tables_database_not_found(self):
        self.login(username="admin")
        uri = f"bridge/tables/invalid/public/"
        rv = self.client.get(uri)
        self.assertEqual(rv.status_code, 404)

    def test_get_bridge_tables_schema_undefined(self):
        example_db = bridge.utils.database.get_example_database()
        self.login(username="gamma")
        uri = f"bridge/tables/{example_db.id}/undefined/"
        rv = self.client.get(uri)
    ...
    def test_save_slice(self):
        ...
        url = (
            "/bridge/explore/table/{}/?slice_name={}&"
    ...
    def test_filter_endpoint(self):
        ...
        url = (
            "/bridge/filter/table/{}/target/?viz_type=sankey&groupby=source"
    ...
    def test_get_user_slices_for_owners(self):
        ...
        url = f"/bridge/user_slices/{user.id}/"
        resp = self.client.get(url)
    ...
    def test_testconn(self, username="admin"):
        ...
        database = bridge.utils.database.get_example_database()
        ...
        response = self.client.post(
            "/bridge/testconn", data=data, content_type="application/json"
        )
    ...
    def test_testconn_unsafe_uri(self, username="admin"):
        ...
        response = self.client.post(
            ...
            data=json.dumps(
                {
                    "uri": "sqlite:///home/bridge/unsafe.db",
                    "name": "unsafe",
                    "impersonate_user": False,
                }
            ),
    ...
    def test_warm_up_cache(self):
        self.login()
        slc = self.get_slice("Girls", db.session)
        data = self.get_json_resp("/bridge/warm_up_cache?slice_id={}".format(slc.id))
        self.assertEqual(
            data, [{"slice_id": slc.id, "viz_error": None, "viz_status": "success"}]
        )

        data = self.get_json_resp(
            "/bridge/warm_up_cache?table_name=energy_usage&db_name=main"
        )
        assert len(data) > 0
        ...
        assert self.get_json_resp(
            f"/bridge/warm_up_cache?dashboard_id={dashboard.id}&slice_id={slc.id}"
        ) == [{"slice_id": slc.id, "viz_error": None, "viz_status": "success"}]

        assert self.get_json_resp(
            f"/bridge/warm_up_cache?dashboard_id={dashboard.id}&slice_id={slc.id}&extra_filters="
    ...
    def test_cache_logging(self):
        ...
        self.get_json_resp("/bridge/warm_up_cache?slice_id={}".format(girls_slice.id))
    ...
    def test_csv_endpoint(self):
        ...
        resp = self.get_resp("/bridge/csv/{}".format(client_id))
    ...
    def test_extra_table_metadata(self):
        self.login()
        example_db = bridge.utils.database.get_example_database()
        schema = "default" if example_db.backend in {"presto", "hive"} else "bridge"
        self.get_json_resp(
            f"/bridge/extra_table_metadata/{example_db.id}/birth_names/{schema}/"
        )
    ...
    def test_templated_sql_json(self):
        if bridge.utils.database.get_example_database().backend == "presto":
    ...
    @mock.patch(
        "tests.integration_tests.bridge_test_custom_template_processors.datetime"
    )
    @mock.patch("bridge.views.core.get_sql_results")
    def test_custom_templated_sql_json(self, sql_lab_mock, mock_dt) -> None:
        ...
        self.get_json_resp(
            "/bridge/sql_json/", raise_on_error=False, json_=json_payload
        )
    ...
    def test_fetch_datasource_metadata(self):
        self.login(username="admin")
        url = "/bridge/fetch_datasource_metadata?" "datasourceKey=1__table"
    ...
    def _get_user_activity_endpoints(user: str):
        ...
        return (
            f"/bridge/recent_activity/{userid}/",
            f"/bridge/created_slices/{userid}/",
            f"/bridge/created_dashboards/{userid}/",
            f"/bridge/fave_slices/{userid}/",
            f"/bridge/fave_dashboards/{userid}/",
            f"/bridge/user_slices/{userid}/",
            f"/bridge/fave_dashboards_by_username/{user}/",
        )
    ...
    def test_user_profile(self, username="admin"):
        ...
        # Setting some faves
        url = f"/bridge/favstar/Slice/{slc.id}/select/"
        resp = self.get_json_resp(url)
        self.assertEqual(resp["count"], 1)

        dash = db.session.query(Dashboard).filter_by(slug="births").first()
        url = f"/bridge/favstar/Dashboard/{dash.id}/select/"
        resp = self.get_json_resp(url)
        self.assertEqual(resp["count"], 1)

        resp = self.get_resp(f"/bridge/profile/{username}/")
    ...
    def test_user_profile_optional_access(self):
        self.login(username="gamma")
        resp = self.client.get(f"/bridge/profile/admin/")
        self.assertEqual(resp.status_code, 200)
    ...
    def test_dataframe_timezone(self):
        ...
        results = BridgeResultSet(list(data), [["data"]], BaseEngineSpec)
    ...
    def test_slice_payload_no_datasource(self):
        self.login(username="admin")
        data = self.get_json_resp("/bridge/explore_json/", raise_on_error=False)
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch.dict(
        "bridge.extensions.feature_flag_manager._feature_flags",
    ...
    def test_explore_json_async_results_format(self):
        ...
        self.login(username="admin")
        rv = self.client.post(
            "/bridge/explore_json/?results=true",
            data={"form_data": json.dumps(form_data)},
        )
        self.assertEqual(rv.status_code, 200)
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch(
        "bridge.utils.cache_manager.CacheManager.cache",
        new_callable=mock.PropertyMock,
    )
    @mock.patch("bridge.viz.BaseViz.force_cached", new_callable=mock.PropertyMock)
    def test_explore_json_data(self, mock_force_cached, mock_cache):
        ...
        self.login(username="admin")
        rv = self.client.get("/bridge/explore_json/data/valid-cache-key")
        data = json.loads(rv.data.decode("utf-8"))
    ...
    def test_explore_json_data_invalid_cache_key(self):
        ...
        rv = self.client.get(f"/bridge/explore_json/data/{cache_key}")
        data = json.loads(rv.data.decode("utf-8"))
    ...
    @mock.patch(
        "bridge.security.BridgeSecurityManager.get_schemas_accessible_by_user"
    )
    @mock.patch("bridge.security.BridgeSecurityManager.can_access_database")
    @mock.patch("bridge.security.BridgeSecurityManager.can_access_all_datasources")
    def test_schemas_access_for_csv_upload_endpoint(
        ...
        data = self.get_json_resp(
            url="/bridge/schemas_access_for_file_upload?db_id={db_id}".format(
                db_id=dbobj.id
            )
        )
    ...
    @mock.patch("bridge.views.core.results_backend_use_msgpack", False)
    def test_display_limit(self):
        from bridge.views import core
        ...
        with mock.patch("bridge.views.core.db") as mock_bridge_db:
            mock_bridge_db.session.query().filter_by().one_or_none.return_value = (
                query_mock
            )
            # get all results
            result_key = json.loads(self.get_resp("/bridge/results/key/"))
            result_limited = json.loads(self.get_resp("/bridge/results/key/?rows=1"))
    ...
    def test_results_default_deserialization(self):
        ...
        results = BridgeResultSet(data, cursor_descr, db_engine_spec)
        query = {
            "database_id": 1,
        ...
        query_mock = mock.Mock()
        deserialized_payload = bridge.views.utils._deserialize_results_payload(
    ...
    def test_feature_flag_serialization(self):
        ...
        urls = [
            "/bridge/sqllab",
            "/bridge/welcome",
            f"/bridge/dashboard/{dash_id}/",
            "/bridge/profile/admin/",
    ...
    def test_sqllab_backend_persistence_payload(self):
        ...
        payload = views.Bridge._get_sqllab_tabs(user_id=user_id)
    ...
    def test_explore_database_id(self):
        database = bridge.utils.database.get_example_database()
        explore_database = bridge.utils.database.get_example_database()
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @mock.patch("bridge.models.core.DB_CONNECTION_MUTATOR")
    def test_explore_injected_exceptions(self, mock_db_connection_mutator):
        ...
        exception = BridgeException("Error message")
    ...
    def test_dashboard_injected_exceptions(self, mock_db_connection_mutator):
        ...
        url = f"/bridge/dashboard/{dash.id}/"
    ...
    @mock.patch("bridge.sql_lab.cancel_query")
    @mock.patch("bridge.views.core.db.session")
    def test_stop_query_not_implemented(
        self, mock_bridge_db_session, mock_sql_lab_cancel_query
    ):
        """
        ...
        self.login(username="admin")
        mock_bridge_db_session.query().filter_by().one().return_value = query_mock
    ...
    @pytest.mark.usefixtures("load_energy_table_with_slice")
    @mock.patch("bridge.explore.form_data.commands.create.CreateFormDataCommand.run")
    def test_explore_redirect(self, mock_command: mock.Mock):
        ...
        rv = self.client.get(
            f"/bridge/explore/?form_data={quote(json.dumps(form_data))}"
        )
