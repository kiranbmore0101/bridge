...
# isort:skip_file
import json
from bridge.utils.core import DatasourceType
import textwrap
import unittest
...
from bridge.utils.core import override_user
...
from bridge import app, db as metadata_db
...
class TestDatabaseModel(BridgeTestCase):
    @unittest.skipUnless(
        BridgeTestCase.is_module_installed("requests"), "requests not installed"
    )
    ...
    @unittest.skipUnless(
        BridgeTestCase.is_module_installed("thrift"), "thrift not installed"
    )
    @unittest.skipUnless(
        BridgeTestCase.is_module_installed("pyhive"), "pyhive not installed"
    )
    ...
    @unittest.skipUnless(
        BridgeTestCase.is_module_installed("MySQLdb"), "mysqlclient not installed"
    )
    def test_database_schema_mysql(self):
        sqlalchemy_uri = "mysql://root@localhost/bridge"
        model = Database(database_name="test_database", sqlalchemy_uri=sqlalchemy_uri)

        with model.get_sqla_engine_with_context() as engine:
            db = make_url(engine.url).database
            self.assertEqual("bridge", db)
    ...
    @mock.patch("bridge.models.core.create_engine")
    ...
    def test_multi_statement(self):
        ...
        if main_db.backend == "mysql":
            df = main_db.get_df("USE bridge; SELECT 1", None)
            self.assertEqual(df.iat[0, 0], 1)

            df = main_db.get_df("USE bridge; SELECT ';';", None)
    ...
    def test_get_sqla_engine(self, mocked_create_engine):
        ...
        model.db_engine_spec.get_dbapi_exception_mapping = mock.Mock(
            return_value={Exception: BridgeException}
        )
...
class TestSqlaTableModel(BridgeTestCase):
