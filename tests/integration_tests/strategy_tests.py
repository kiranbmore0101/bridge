...
# under the License.
# isort:skip_file
"""Unit tests for Bridge cache warmup"""
import datetime
import json
...
from bridge.models.core import Log
from bridge.tags.models import get_tag, ObjectTypes, TaggedObject, TagTypes
from bridge.tasks.cache import (
...
class TestCacheWarmUp(BridgeTestCase):
    ...
    def test_top_n_dashboards_strategy(self):
        ...
        for _ in range(10):
            self.client.get(f"/bridge/dashboard/{dash.id}/")

        strategy = TopNDashboardsStrategy(1)
        result = sorted(strategy.get_urls())
        expected = sorted(
            [
                f"{get_url_host()}bridge/warm_up_cache/?slice_id={slc.id}&dashboard_id={dash.id}"
    ...
    def test_dashboard_tags(self):
        ...
        tag1_urls = sorted(
            [
                f"{get_url_host()}bridge/warm_up_cache/?slice_id={slc.id}"
                for slc in dash.slices
            ]
        )
        ...
        slc = dash.slices[0]
        tag2_urls = [f"{get_url_host()}bridge/warm_up_cache/?slice_id={slc.id}"]
        object_id = slc.id
        tagged_object = TaggedObject(
