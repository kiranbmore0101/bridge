...
from bridge.connectors.sqla.models import SqlaTable, SqlMetric, TableColumn
from bridge.constants import CacheRegion
from bridge.dao.exceptions import DatasourceNotFound, DatasourceTypeNotSupportedError
from bridge.datasets.commands.exceptions import DatasetNotFoundError
from bridge.exceptions import BridgeGenericDBErrorException
from bridge.models.core import Database
from bridge.utils.core import backend, get_example_default_schema
from bridge.utils.database import get_example_database, get_main_database
from tests.integration_tests.base_tests import db_insert_temp_object, BridgeTestCase
...
class TestDatasource(BridgeTestCase):
    def setUp(self):
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch("bridge.connectors.sqla.models.SqlaTable.external_metadata")
    def test_external_metadata_error_return_400(self, mock_get_datasource):
        self.login(username="admin")
        tbl = self.get_table(name="birth_names")
        url = f"/datasource/external_metadata/table/{tbl.id}/"

        mock_get_datasource.side_effect = BridgeGenericDBErrorException("oops")

        pytest.raises(
            BridgeGenericDBErrorException,
    ...
    def test_save_default_endpoint_validation_success(self):
        ...
        datasource_post["id"] = tbl_id
        datasource_post["owners"] = [1]
        datasource_post["default_endpoint"] = "http://localhost/bridge/1"
        data = dict(data=json.dumps(datasource_post))
        resp = self.client.post("/datasource/save/", data=data)
        assert resp.status_code == 200
