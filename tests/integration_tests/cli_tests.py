...
import bridge.cli.importexport
...
@mock.patch.dict(
    "bridge.cli.lib.feature_flags", {"VERSIONED_EXPORT": False}, clear=True
)
...
def test_export_dashboards_original(app_context, fs):
    ...
    import bridge.cli.importexport  # noqa: F811

    # reload to define export_dashboards correctly based on the
    # feature flags
    importlib.reload(bridge.cli.importexport)

    runner = app.test_cli_runner()
    response = runner.invoke(
        bridge.cli.importexport.export_dashboards, ("-f", "dashboards.json")
    )
...
def test_export_datasources_original(app_context, fs):
    ...
    runner = app.test_cli_runner()
    response = runner.invoke(
        bridge.cli.importexport.export_datasources, ("-f", "datasources.yaml")
    )
...
@pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
@mock.patch.dict(
    "bridge.cli.lib.feature_flags", {"VERSIONED_EXPORT": True}, clear=True
)
def test_export_dashboards_versioned_export(app_context, fs):
    ...
    with freeze_time("2021-01-01T00:00:00Z"):
        response = runner.invoke(bridge.cli.importexport.export_dashboards, ())
...
@mock.patch(
    "bridge.dashboards.commands.export.ExportDashboardsCommand.run",
...
def test_export_datasources_versioned_export(app_context, fs):
    ...
    runner = app.test_cli_runner()
    with freeze_time("2021-01-01T00:00:00Z"):
        response = runner.invoke(bridge.cli.importexport.export_datasources, ())
...
@mock.patch(
    "bridge.dashboards.commands.export.ExportDatasetsCommand.run",
    side_effect=Exception(),
)
...
@mock.patch("bridge.dashboards.commands.importers.dispatcher.ImportDashboardsCommand")
def test_import_dashboards_versioned_export(import_dashboards_command, app_context, fs):
    ...
    runner = app.test_cli_runner()
    response = runner.invoke(
        bridge.cli.importexport.import_dashboards, ("-p", "dashboards.json")
    )
    ...
    runner = app.test_cli_runner()
    response = runner.invoke(
        bridge.cli.importexport.import_dashboards, ("-p", "dashboards.zip")
    )
...
@mock.patch(
    "bridge.dashboards.commands.importers.dispatcher.ImportDashboardsCommand.run",
    side_effect=Exception(),
)
...
@mock.patch("bridge.datasets.commands.importers.dispatcher.ImportDatasetsCommand")
def test_import_datasets_versioned_export(import_datasets_command, app_context, fs):
    ...
    runner = app.test_cli_runner()
    response = runner.invoke(
        bridge.cli.importexport.import_datasources, ("-p", "datasets.yaml")
    )
    ...
    runner = app.test_cli_runner()
    response = runner.invoke(
        bridge.cli.importexport.import_datasources, ("-p", "datasets.zip")
    )
...
@mock.patch("bridge.datasets.commands.importers.v0.ImportDatasetsCommand")
def test_import_datasets_sync_argument_columns_metrics(
    import_datasets_command, app_context, fs
):
    """
    Test that the --sync command line argument syncs dataset in bridge
    ...
    runner = app.test_cli_runner()
    response = runner.invoke(
        bridge.cli.importexport.import_datasources,
...
@mock.patch(
    "bridge.datasets.commands.importers.dispatcher.ImportDatasetsCommand.run",
    side_effect=Exception(),
)
