...
from bridge import db, security_manager
from bridge.connectors.sqla.models import SqlaTable
from bridge.db_engine_specs import BaseEngineSpec
from bridge.db_engine_specs.hive import HiveEngineSpec
from bridge.db_engine_specs.presto import PrestoEngineSpec
from bridge.errors import ErrorLevel, BridgeError, BridgeErrorType
from bridge.exceptions import BridgeErrorException
from bridge.models.sql_lab import Query, SavedQuery
from bridge.result_set import BridgeResultSet
from bridge.sqllab.limiting_factor import LimitingFactor
from bridge.sql_lab import (
    cancel_query,
    execute_sql_statements,
    apply_limit_if_exists,
)
from bridge.sql_parse import CtasMethod
from bridge.utils.core import (
    backend,
    datetime_to_epoch,
)
from bridge.utils.database import get_example_database, get_main_database
...
class TestSqlLab(BridgeTestCase):
    ...
    def test_sql_json(self):
        ...
        if backend() == "presto":
            assert (
                ...
                == BridgeErrorType.TABLE_DOES_NOT_EXIST_ERROR
            )
            ...
            "errors": [
                {
                    ...
                    "error_type": BridgeErrorType.DML_NOT_ALLOWED_ERROR,
    ...
    def test_sql_json_cta_dynamic_db(self, ctas_method):
        ...
        with mock.patch(
            "bridge.sqllab.sqllab_execution_context.get_cta_schema_name",
    ...
    def test_queries_endpoint(self):
        ...
        resp = self.client.get("/bridge/queries/0")
        # Redirects to the login page
        self.assertEqual(401, resp.status_code)

        # Admin sees queries
        self.login("admin")
        data = self.get_json_resp("/bridge/queries/0")
        self.assertEqual(2, len(data))
        data = self.get_json_resp("/bridge/queries/0.0")
        self.assertEqual(2, len(data))
        ...
        data = self.get_json_resp(
            "/bridge/queries/{}".format(float(datetime_to_epoch(now)) - 1000)
        )
    ...
    def test_search_query_on_db_id(self):
        ...
        data = self.get_json_resp(
            f"/bridge/search_queries?database_id={examples_dbid}"
        )
        self.assertEqual(3, len(data))
        db_ids = [k["dbId"] for k in data]
        self.assertEqual([examples_dbid for i in range(3)], db_ids)

        resp = self.get_resp("/bridge/search_queries?database_id=-1")
    ...
    def test_search_query_on_user(self):
        ...
        data = self.get_json_resp("/bridge/search_queries?user_id={}".format(user_id))
        self.assertEqual(2, len(data))
        user_ids = {k["userId"] for k in data}
        self.assertEqual(set([user_id]), user_ids)

        user_id = security_manager.find_user("gamma_sqllab").id
        resp = self.get_resp("/bridge/search_queries?user_id={}".format(user_id))
        data = json.loads(resp)
        self.assertEqual(1, len(data))
    ...
    def test_search_query_on_status(self):
        ...
        resp = self.get_resp("/bridge/search_queries?status=success")
        data = json.loads(resp)
        self.assertEqual(2, len(data))
        states = [k["state"] for k in data]
        self.assertEqual(["success", "success"], states)

        resp = self.get_resp("/bridge/search_queries?status=failed")
        data = json.loads(resp)
        self.assertEqual(1, len(data))
    ...
    def test_search_query_on_text(self):
        self.run_some_queries()
        self.login("admin")
        url = "/bridge/search_queries?search_text=birth"
    ...
    def test_search_query_filter_by_time(self):
        ...
        url = f"/bridge/search_queries?from={from_time}&to={to_time}"
    ...
    def test_search_query_only_owned(self) -> None:
        ...
        user_id = security_manager.find_user("gamma_sqllab").id
        data = self.get_json_resp("/bridge/search_queries")
    ...
    def test_ps_conversion_no_dict(self):
        ...
        data = [["a", 4, 4.0]]
        results = BridgeResultSet(data, cols, BaseEngineSpec)
    ...
    def test_sqllab_viz(self):
        ...
        resp = self.get_json_resp("/bridge/sqllab_viz/", data=data)
    ...
    def test_sqllab_table_viz(self):
        ...
        data = {"data": json.dumps(payload)}
        resp = self.get_json_resp("/bridge/get_or_create_table/", data=data)
        self.assertIn("table_id", resp)
    ...
    @mock.patch("bridge.sql_lab.get_query")
    @mock.patch("bridge.sql_lab.execute_sql_statement")
    ...
    @mock.patch("bridge.sql_lab.results_backend", None)
    @mock.patch("bridge.sql_lab.get_query")
    @mock.patch("bridge.sql_lab.execute_sql_statement")
    def test_execute_sql_statements_no_results_backend(
        ...
        with pytest.raises(BridgeErrorException) as excinfo:
            execute_sql_statements(
                query_id=1,
                rendered_query=sql,
                return_results=True,
        ...
        assert excinfo.value.error == BridgeError(
    ...
    def test_execute_sql_statements_ctas(
        ...
        assert excinfo.value.error == BridgeError(
            ...
            error_type=BridgeErrorType.INVALID_CTAS_QUERY_ERROR,
            level=ErrorLevel.ERROR,
            extra={
                "issue_codes": [
                    {
        ...
        assert excinfo.value.error == BridgeError(
            ...
            error_type=BridgeErrorType.INVALID_CVAS_QUERY_ERROR,
            level=ErrorLevel.ERROR,
            extra={
                "issue_codes": [
                    {
            ...
            "errors": [
                {
                    ...
                    "error_type": BridgeErrorType.SQLLAB_TIMEOUT_ERROR,
                    "level": ErrorLevel.ERROR,
                    "extra": {
                        "issue_codes": [
                            {
