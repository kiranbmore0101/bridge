...
from bridge.reports.models import ReportSourceFormat
...
@patch(
    "bridge.reports.commands.execute.DashboardScreenshot",
)
@patch(
    "bridge.dashboards.permalink.commands.create.CreateDashboardPermalinkCommand.run"
)
def test_report_for_dashboard_with_tabs(
    ...
    with create_dashboard_report(
        ...
        assert url.endswith(f"/bridge/dashboard/p/{permalink_key}/")
