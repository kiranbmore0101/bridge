...
from bridge.reports.commands.execute import (
    AsyncExecuteReportScheduleCommand,
    BaseReportState,
)
from bridge.reports.commands.log_prune import AsyncPruneReportScheduleLogCommand
from bridge.reports.models import (
    ReportDataFormat,
    ReportExecutionLog,
    ReportSchedule,
    ReportScheduleType,
...
from bridge.reports.notifications.exceptions import (
    NotificationError,
...
@patch("bridge.reports.notifications.email.send_email_smtp")
@patch("bridge.utils.screenshots.ChartScreenshot.get_screenshot")
def test_email_chart_report_schedule(
    ...
    with freeze_time("2020-01-01T00:00:00Z"):
        ...
        assert (
            ...
            'standalone=0&force=false">Explore in Bridge</a>'
...
def test_email_chart_report_schedule_force_screenshot(
    ...
    with freeze_time("2020-01-01T00:00:00Z"):
        ...
        assert (
            ...
            'standalone=0&force=true">Explore in Bridge</a>'
            in email_mock.call_args[0][2]
        )
        # Assert the email smtp address
        assert email_mock.call_args[0][0] == notification_targets[0]
...
@patch("bridge.utils.csv.urllib.request.urlopen")
@patch("bridge.utils.csv.urllib.request.OpenerDirector.open")
@patch("bridge.reports.notifications.email.send_email_smtp")
@patch("bridge.utils.csv.get_chart_csv_data")
...
@patch("bridge.utils.csv.urllib.request.OpenerDirector.open")
@patch("bridge.reports.notifications.email.send_email_smtp")
@patch("bridge.utils.csv.get_chart_dataframe")
...
@patch("bridge.reports.notifications.email.send_email_smtp")
@patch("bridge.utils.screenshots.DashboardScreenshot.get_screenshot")
...
@patch("bridge.reports.notifications.slack.WebClient.files_upload")
@patch("bridge.utils.screenshots.ChartScreenshot.get_screenshot")
...
@patch("bridge.reports.notifications.slack.WebClient.chat_postMessage")
@patch("bridge.utils.csv.urllib.request.urlopen")
@patch("bridge.utils.csv.urllib.request.OpenerDirector.open")
@patch("bridge.utils.csv.get_chart_dataframe")
...
|  1 | c21  | c22  | c23       |"""
        ...
        assert (
            f"<http://0.0.0.0:8080/explore/?form_data=%7B%22slice_id%22%3A%20{create_report_slack_chart_with_text.chart.id}%7D&standalone=0&force=false|Explore in Bridge>"
...
@patch("bridge.reports.notifications.slack.WebClient")
@patch("bridge.utils.screenshots.ChartScreenshot.get_screenshot")
...
def test_soft_timeout_alert(email_mock, create_alert_email_chart):
    ...
    from bridge.reports.commands.exceptions import AlertQueryTimeout
...
@patch("bridge.reports.dao.ReportScheduleDAO.bulk_delete_logs")
...
@patch("bridge.reports.commands.execute.logger")
@patch("bridge.reports.commands.execute.create_notification")
def test__send_with_client_errors(notification_mock, logger_mock):
    ...
    assert excinfo.errisinstance(BridgeException)
    logger_mock.warning.assert_called_with(
        (
            "BridgeError(message='', error_type=<BridgeErrorType.REPORT_NOTIFICATION_ERROR: 'REPORT_NOTIFICATION_ERROR'>, level=<ErrorLevel.WARNING: 'warning'>, extra=None)"
        )
    )
...
def test__send_with_multiple_errors(notification_mock, logger_mock):
    ...
    logger_mock.warning.assert_has_calls(
        [
            ...
            call(
                "BridgeError(message='', error_type=<BridgeErrorType.REPORT_NOTIFICATION_ERROR: 'REPORT_NOTIFICATION_ERROR'>, level=<ErrorLevel.ERROR: 'error'>, extra=None)"
            ),
        ]
    )
