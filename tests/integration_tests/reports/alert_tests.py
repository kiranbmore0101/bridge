...
from bridge.reports.commands.exceptions import AlertQueryError
from bridge.reports.models import ReportCreationMethod, ReportScheduleType
...
def test_execute_query_as_report_executor(
    ...
    with app.app_context():
        ...
        override_user_mock = mocker.patch(
            "bridge.reports.commands.alert.override_user"
        )
        cm = (
...
def test_execute_query_succeeded_no_retry(
    ...
    execute_query_mock = mocker.patch(
        "bridge.reports.commands.alert.AlertCommand._execute_query",
...
def test_execute_query_succeeded_with_retries(
    ...
    from bridge.reports.commands.alert import AlertCommand, AlertQueryError
    ...
    # Should match the value defined in bridge_test_config.py
...
def test_execute_query_failed_no_retry(mocker: MockFixture, app_context: None) -> None:
    from bridge.reports.commands.alert import AlertCommand, AlertQueryTimeout
