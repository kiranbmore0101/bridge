...
if TYPE_CHECKING:
    from bridge.extensions.metastore_cache import BridgeMetastoreCache
...
@pytest.fixture
def cache() -> BridgeMetastoreCache:
    from bridge.extensions.metastore_cache import BridgeMetastoreCache

    return BridgeMetastoreCache(
        namespace=UUID("ee173d1b-ccf3-40aa-941c-985c15224496"),
        default_timeout=600,
    )


def test_caching_flow(app_context: AppContext, cache: BridgeMetastoreCache) -> None:
...
def test_expiry(app_context: AppContext, cache: BridgeMetastoreCache) -> None:
