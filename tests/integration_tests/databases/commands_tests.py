...
from bridge import db, event_logger, security_manager
from bridge.commands.exceptions import CommandInvalidError
from bridge.commands.importers.exceptions import IncorrectVersionError
from bridge.connectors.sqla.models import SqlaTable
from bridge.databases.commands.create import CreateDatabaseCommand
from bridge.databases.commands.exceptions import (
...
from bridge.databases.commands.export import ExportDatabasesCommand
from bridge.databases.commands.importers.v1 import ImportDatabasesCommand
from bridge.databases.commands.test_connection import TestConnectionDatabaseCommand
from bridge.databases.commands.validate import ValidateDatabaseParametersCommand
from bridge.databases.schemas import DatabaseTestConnectionSchema
...
from bridge.utils.core import backend
...
class TestCreateDatabaseCommand(BridgeTestCase):
    @mock.patch(
        "bridge.databases.commands.test_connection.event_logger.log_with_context"
    )
    @mock.patch("bridge.utils.core.g")
...
class TestExportDatabasesCommand(BridgeTestCase):
    @skip("Flaky")
...
class TestImportDatabasesCommand(BridgeTestCase):
    ...
    @patch("bridge.databases.commands.importers.v1.import_dataset")
...
class TestTestConnectionDatabaseCommand(BridgeTestCase):
    @mock.patch("bridge.databases.dao.Database._get_sqla_engine")
    @mock.patch(
        "bridge.databases.commands.test_connection.event_logger.log_with_context"
    )
    @mock.patch("bridge.utils.core.g")
    ...
    def test_connection_do_ping_exception(
        ...
        with pytest.raises(BridgeErrorsException) as excinfo:
            command_without_db_name.run()
        assert (
    ...
    @mock.patch("bridge.databases.commands.test_connection.func_timeout")
    @mock.patch(
        "bridge.databases.commands.test_connection.event_logger.log_with_context"
    )
    @mock.patch("bridge.utils.core.g")
    def test_connection_do_ping_timeout(
        ...
        with pytest.raises(BridgeTimeoutException) as excinfo:
            command_without_db_name.run()
        assert excinfo.value.status == 408
        assert (
            excinfo.value.error.error_type
            == BridgeErrorType.CONNECTION_DATABASE_TIMEOUT
        )
    ...
    @mock.patch("bridge.utils.core.g")
    def test_connection_bridge_security_connection(
        self, mock_g, mock_event_logger, mock_get_sqla_engine
    ):
        ...
        database = get_example_database()
        mock_g.user = security_manager.find_user("admin")
        mock_get_sqla_engine.side_effect = BridgeSecurityException(
            BridgeError(error_type=500, message="test", level="info")
        )
        db_uri = database.sqlalchemy_uri_decrypted
        json_payload = {"sqlalchemy_uri": db_uri}
        command_without_db_name = TestConnectionDatabaseCommand(json_payload)
...
@mock.patch("bridge.db_engine_specs.base.is_hostname_valid")
@mock.patch("bridge.db_engine_specs.base.is_port_open")
@mock.patch("bridge.databases.commands.validate.DatabaseDAO")
...
def test_validate_partial(is_port_open, is_hostname_valid, app_context):
    ...
    assert excinfo.value.errors == [
        BridgeError(
            ...
            error_type=BridgeErrorType.CONNECTION_MISSING_PARAMETERS_ERROR,
...
def test_validate_partial_invalid_hostname(is_hostname_valid, app_context):
    ...
    assert excinfo.value.errors == [
        ...
        BridgeError(
            ...
            error_type=BridgeErrorType.CONNECTION_INVALID_HOSTNAME_ERROR,
