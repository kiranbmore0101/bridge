...
from bridge.db_engine_specs.mysql import MySQLEngineSpec
from bridge.db_engine_specs.postgres import PostgresEngineSpec
from bridge.db_engine_specs.redshift import RedshiftEngineSpec
from bridge.db_engine_specs.bigquery import BigQueryEngineSpec
from bridge.db_engine_specs.gsheets import GSheetsEngineSpec
from bridge.db_engine_specs.hana import HanaEngineSpec
from bridge.errors import BridgeError
from bridge.models.core import Database, ConfigurationMethod
from bridge.reports.models import ReportSchedule, ReportScheduleType
...
class TestDatabaseApi(BridgeTestCase):
    def insert_database(
        self,
        database_name: str,
        sqlalchemy_uri: str,
    ...
    def test_create_database_conn_fail(self):
        ...
        response = self.client.post(uri, json=database_data)
        response_data = json.loads(response.data.decode("utf-8"))
        bridge_error_mysql = BridgeError(
            message='Either the username "bridge" or the password is incorrect.',
        ...
        bridge_error_postgres = BridgeError(
            message='The password provided for username "bridge" is incorrect.',
        ...
        expected_response_mysql = {"errors": [dataclasses.asdict(bridge_error_mysql)]}
        expected_response_postgres = {
            "errors": [dataclasses.asdict(bridge_error_postgres)]
        }
            ...
            "errors": [
                {
                    ...
                    "extra": {
                        "issue_codes": [
                            {
                                "code": 1010,
                                "message": "Issue 1010 - Bridge encountered an error while running a command.",
                            }
                        ]
                    },
                }
    ...
    def test_test_connection_unsafe_uri(self):
        ...
        app.config["PREVENT_UNSAFE_DB_CONNECTIONS"] = True
        data = {
            "sqlalchemy_uri": "sqlite:///home/bridge/unsafe.db",
    ...
    @mock.patch(
        "bridge.databases.commands.test_connection.DatabaseDAO.build_db_for_connection_test",
    )
    @mock.patch(
        "bridge.databases.commands.test_connection.event_logger",
    )
    def test_test_connection_failed_invalid_hostname(
        ...
        response = json.loads(rv.data.decode("utf-8"))
        expected_response = {"errors": [dataclasses.asdict(bridge_error)]}
                        ...
                        "issue_codes": [
                            {
                                "code": 1010,
                                "message": (
                                    "Issue 1010 - Bridge encountered an "
    ...
    @mock.patch(
        "bridge.db_engine_specs.base.BaseEngineSpec.get_function_names",
    )
    ...
    @mock.patch("bridge.databases.api.get_available_engine_specs")
    @mock.patch("bridge.databases.api.app")
    ...
    @mock.patch("bridge.databases.api.ValidateDatabaseParametersCommand")
    ...
    @patch.dict(
        "bridge.config.SQL_VALIDATORS_BY_ENGINE",
        SQL_VALIDATORS_BY_ENGINE,
        clear=True,
    )
    ...
    @patch("bridge.databases.commands.validate_sql.get_validator_by_name")
    @patch.dict(
        "bridge.config.SQL_VALIDATORS_BY_ENGINE",
        PRESTO_SQL_VALIDATORS_BY_ENGINE,
        clear=True,
