...
import bridge.viz as viz
from bridge import app
from bridge.constants import NULL_STRING
from bridge.exceptions import QueryObjectValidationError, SpatialException
from bridge.utils.core import DTTM_ALIAS
...
class TestBaseViz(BridgeTestCase):
...
class TestTableViz(BridgeTestCase):
    ...
    @patch("bridge.viz.BaseViz.query_obj")
...
class TestDistBarViz(BridgeTestCase):
...
class TestPairedTTest(BridgeTestCase):
...
class TestPartitionViz(BridgeTestCase):
    @patch("bridge.viz.BaseViz.query_obj")
...
class TestRoseVis(BridgeTestCase):
...
class TestTimeSeriesTableViz(BridgeTestCase):
...
class TestBaseDeckGLViz(BridgeTestCase):
...
class TestTimeSeriesViz(BridgeTestCase):
...
class TestBigNumberViz(BridgeTestCase):
...
class TestPivotTableViz(BridgeTestCase):
    df = pd.DataFrame(
        data={
            "intcol": [1, 2, 3, None],
...
class TestFilterBoxViz(BridgeTestCase):
    def test_get_data(self):
        form_data = {
            "filter_configs": [
