...
from bridge import db, is_feature_enabled, security_manager
from bridge.extensions import machine_auth_provider_factory
from bridge.models.dashboard import Dashboard
from bridge.models.slice import Slice
from bridge.utils.screenshots import ChartScreenshot, DashboardScreenshot
from bridge.utils.urls import get_url_host, get_url_path
from bridge.utils.webdriver import WebDriverProxy
...
class TestThumbnailsSeleniumLive(LiveServerTestCase):
    ...
    def test_get_async_dashboard_screenshot(self):
        ...
        with patch("bridge.dashboards.api.DashboardRestApi.get") as mock_get:
...
class TestWebDriverProxy(BridgeTestCase):
    @patch("bridge.utils.webdriver.WebDriverWait")
    @patch("bridge.utils.webdriver.firefox")
    @patch("bridge.utils.webdriver.sleep")
    def test_screenshot_selenium_headstart(
        ...
        url = get_url_path("Bridge.slice", slice_id=1, standalone="true")
...
class TestThumbnails(BridgeTestCase):
    ...
    def test_get_async_dashboard_screenshot(self):
        ...
        uri = f"api/v1/dashboard/{dashboard.id}/thumbnail/{dashboard.digest}/"
        with patch(
            "bridge.tasks.thumbnails.cache_dashboard_thumbnail.delay"
        ) as mock_task:
            rv = self.client.get(uri)
            self.assertEqual(rv.status_code, 202)

            expected_uri = f"{get_url_host()}bridge/dashboard/{dashboard.id}/"
    ...
    def test_get_async_chart_screenshot(self):
        ...
        with patch(
            "bridge.tasks.thumbnails.cache_chart_thumbnail.delay"
        ) as mock_task:
            rv = self.client.get(uri)
            self.assertEqual(rv.status_code, 202)
            expected_uri = f"{get_url_host()}bridge/slice/{chart.id}/?standalone=true"
