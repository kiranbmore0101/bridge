...
from bridge.views.base_api import BaseBridgeModelRestApi, requires_json

from .base_tests import BridgeTestCase
from .conftest import with_config


class Model1Api(BaseBridgeModelRestApi):
...
class TestOpenApiSpec(BridgeTestCase):
...
class TestBaseModelRestApi(BridgeTestCase):
...
class ApiOwnersTestCaseMixin:
    ...
    @patch(
        "bridge.security.BridgeSecurityManager.get_exclude_users_from_lists",
        return_value=["gamma"],
    )
