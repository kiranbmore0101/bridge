...
from bridge.connectors.sqla.models import TableColumn, SqlaTable
from bridge.errors import BridgeErrorType
from bridge.extensions import async_query_manager, db
from bridge.models.annotations import AnnotationLayer
from bridge.models.slice import Slice
from bridge.bridge_typing import AdhocColumn
...
class BaseTestChartDataApi(BridgeTestCase):
...
class TestPostChartDataApi(BaseTestChartDataApi):
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch(
        "bridge.common.query_context_factory.config",
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch(
        "bridge.utils.core.current_app.config",
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch(
        "bridge.common.query_actions.config",
...
class TestGetChartDataApi(BaseTestChartDataApi):
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @with_feature_flags(GLOBAL_ASYNC_QUERIES=True)
    @mock.patch("bridge.charts.data.api.QueryContextCacheLoader")
...
@mock.patch(
    "bridge.common.query_context_processor.config",
    {
        **app.config,
