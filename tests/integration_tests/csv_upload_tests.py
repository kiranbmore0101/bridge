...
# under the License.
# isort:skip_file
"""Unit tests for Bridge CSV upload"""
import json
import logging
import os
import shutil
...
def setup_csv_upload(login_as_admin):
    upload_db = bridge.utils.database.get_or_create_db(
        CSV_UPLOAD_DATABASE, app.config["SQLALCHEMY_EXAMPLES_URI"]
    )
    extra = upload_db.get_extra()
    extra["explore_database_id"] = bridge.utils.database.get_example_database().id
...
def mock_upload_to_s3(filename: str, upload_prefix: str, table: Table) -> str:
    ...
    src = os.path.join("/tmp/bridge_uploads", os.path.basename(filename))
    # hdfs destination for the external tables
    dest_dir = os.path.join("/tmp/external/bridge_uploads/", str(table))
